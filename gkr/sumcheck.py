...
_sage_const_16 = Integer(16); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_31 = Integer(31); _sage_const_10 = Integer(10)
...
class SumCheckProtocol:
    ...
    def fiat_shamir_sample(self, input_string: str) -> Fp:
        ...
        return self.F(int(fs_hash, _sage_const_16 ))
    ...
    def prove(self) -> dict:
        ...
        for j in range(self.degree):
            ...
            for i in range(_sage_const_2 **(self.degree-j-_sage_const_1 )):
                # Convert i to a binary representation of length degree-j-1
                binary_representation = [(i >> k) & _sage_const_1  for k in range(self.degree-j-_sage_const_1 )]
    ...
    def verify(self, transcript: dict) -> bool:
        ...
        for i in range(self.degree):
            if random_values[i] != self.fiat_shamir_from_polynomials(polynomials[:i+_sage_const_1 ]):
        ...
        for j in range(self.degree):
            ...
            if j == _sage_const_0 :
                s_1 = polynomials[_sage_const_0 ]
                print(s_1)
                if s_1(_sage_const_0 ) + s_1(_sage_const_1 ) != H:
                    print(f"First round sum {s_1(_sage_const_0 ) + s_1(_sage_const_1 )} does not match the claimed sum {H}.")
                    return False
                continue
            
            # For subsequent rounds, check that the sum of the polynomials matches the claimed sum
            s, s_previous = polynomials[j], polynomials[j-_sage_const_1 ]
            if s(_sage_const_0 ) + s(_sage_const_1 ) != s_previous(random_values[j-_sage_const_1 ]):
                print(f"Round {j} sum {s(_sage_const_0 ) + s(_sage_const_1 )} does not match the previous round's output {s_previous(random_values[j-_sage_const_1 ])}.")
                return False

        # Final round checks whether f(r) = s_v(r_v)
        last_polynomial = polynomials[-_sage_const_1 ]
        last_random = random_values[-_sage_const_1 ]
...
def boolean_hypercube_sum(F, f: PolynomialRing, degree: int) -> GF:
    ...
    total_sum = F(_sage_const_0 )
    for i in range(_sage_const_1 <<degree):
        # Convert i to a binary representation of length 'degree'
        binary_representation = [(i >> j) & _sage_const_1  for j in range(degree)]
        # Evaluate the polynomial at this point
...
if __name__ == "__main__":
    ...
    p = (_sage_const_1 <<_sage_const_31 ) - _sage_const_1 
    Fp = GF(p)

    # Defining the multivariate polynomial ring over the finite field GF(p)
    v = _sage_const_10  # Degree of the polynomial
