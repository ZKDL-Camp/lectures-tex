\documentclass{gkr-template}

\usepackage{minted}
% Style for minted
\setminted{
  fontfamily=cmtt,
  frame=lines,      % Adding a frame for style
  fontsize=\small   % Using a slightly smaller font for code
}
...
\begin{center}
    \textbf{\Huge{GKR and Sumcheck Protocols Overview}}
...
$r_1 \xleftarrow{R} \mathbb{F}$. In such case, the soundness of such check is
$1-\deg f_1/|\mathbb{F}|$. 
...
f_2(r_2)$ at random point $r_2 \xleftarrow{R} \mathbb{F}$. Since computing
...
\subsection*{Sum-Check Protocol Implementation}
...
\begin{align*}
    ...
    s_{10}(X) &= 2030644729X^4 + 1291949229X^3 + 560585988X + 1720313399, &&& r_{10} &= 1461328437
\end{align*}

\section*{Sumcheck Applications}
...
\subsection*{Matrix Multiplication Verification (MatMul Check-Sum Protocol)}
...
$\boldsymbol{\alpha} \xleftarrow{R} \mathbb{F}^n$ and verify that
...
$\boldsymbol{r}_2 \xleftarrow{R} \mathbb{F}^{\log n}$, and apply the sum-check
...
first sample randomnesses $\boldsymbol{r}_1, \boldsymbol{r}_2 \xleftarrow{R}
\mathbb{F}_{11}^2$:
\begin{minted}{python}
...
\begin{align*}
    \widetilde{\mathsf{add}}_2(X_1,X_2,Y_1,Y_2,Y_3,Z_1,Z_2,Z_3) &= (1-X_1)(1-X_2)(1-Y_1)(1-Y_2)(1-Y_3)(1-Z_1)(1-Z_2)Z_3 \\
...
\textbf{Protocol.} The GKR protocol consists of $d$ rounds, one for each layer
of the circuit. In the $i$-th round, the prover $\mathcal{P}$ claims a value for
$\widetilde{W}_i(\mathbf{r}_i)$ at the randomly selected point $\mathbf{r}_i
\xleftarrow{R} \mathbb{F}^{v_i}$. 
...
random point $\mathbf{r}_0 \xleftarrow{R} \mathbb{F}^{v_0}$ and evaluates
...
$\mathbf{r}_{i+1} \xleftarrow{R} \mathbb{F}^{v_{i+1}}$. How? Consider the
following lemma.

\begin{lemma}
    The following statement holds:
    \begin{equation*}
        \widetilde{W}_i(\mathbf{z}) = \sum_{\boldsymbol{b},\boldsymbol{c} \in \{0,1\}^{v_{i+1}}} \widetilde{\mathsf{add}}_i(\mathbf{z},\boldsymbol{a},\boldsymbol{b})(\widetilde{W}_{i+1}(\boldsymbol{b}) + \widetilde{W}_{i+1}(\boldsymbol{c})) + \widetilde{\mathsf{mul}}_i(\mathbf{z},\boldsymbol{b},\boldsymbol{c})\widetilde{W}_{i+1}(\boldsymbol{b})\widetilde{W}_{i+1}(\boldsymbol{c}).
    \end{equation*}
\end{lemma}
...
selected point $(\boldsymbol{b}^*, \boldsymbol{c}^*) \xleftarrow{R}
...
\newpage
\begin{proposition}
Let us summarize the GKR protocol so far:
\begin{itemize}
    ...
    \item $\mathcal{V}$ picks a random point $\mathbf{r}_0 \xleftarrow{R}
    ...
    \begin{itemize}
        ...
        \item $\mathcal{V}$ chooses a random point $\mathbf{r}^* \xleftarrow{R}
