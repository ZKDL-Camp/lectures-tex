...
Essentially, the main group being used for cryptographic protocols is the group
of points on an elliptic curve. If elliptic curve is ``good enough'', then the
discrete logarithm problem assumption, Diffie-Hellman assumption and other core
cryptographic assumptions hold. Moreover, this group does not require a large
field size, which is a huge advantage for many cryptographic protocols.

So, let us formally define what an elliptic curve is. Further assume that, when speaking of the finite field $\mathbb{F}_p$, the underlying prime number is greater than $3$\footnote{Note that, for example, for $\mathbb{F}_{2^n}$ equation of elliptic curve is very different, but usually we do not deal with binary field extensions.}. The definition is the following.
...
\begin{remark}
    One might rightfully ask why do we require $4a^3+27b^2 \neq 0$. This is due to the fact that the curve $y^2=x^3+ax+b$ might have certain degeneracies and special points (namely, \textit{singular} points). So we require this condition to make $E/\mathbb{K}$ ``smooth enough''.
\end{remark}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{contents/images/lecture_3/ec_illustration_1.pdf}
    \caption{Elliptic curve $E/\mathbb{Q}: y^2=x^3-x+9$ with points $P=(0,3),Q=(-1,-3)$ depicted on it.}
    \label{fig:ec_1}
\end{figure}

Typically, our elliptic curve is defined over a finite field $\mathbb{F}_p$, so further feel free to assume that $\mathbb{K}=\mathbb{F}_p$.

\begin{remark}
    In many cases one might encounter the definition where an elliptic curve $E$ is defined over the algebraic closure of $\mathbb{F}_p$, that is $E/\overline{\mathbb{F}}_p$.
    The primary reason is that studying geometric objects in algebra is significantly easier when working in the algebraically closed fields (primarily thanks to the so-called \textit{Hilbert's Nullstellensatz Theorem}).
\end{remark}
...
Now, for cryptographic applications, elliptic curves are useless without any operation defined on top of them. 
Consequently, our goal is to equip $E/\mathbb{K}$ with an operation that will turn $E$ into a group.
But as will be seen later, it is quite unclear how to define the identity element. 
For that reason, we introduce a slightly different definition of a set of points on $E$ than what one might anticipate.

\begin{definition}
    The set of points on an elliptic curve, denoted $E_{a,b}(\mathbb{K})$, is defined as:
    \begin{equation*}
...
\begin{remark}
    The difference between $E(\mathbb{K})$ and $E/\mathbb{K}$ is that the former includes the point at infinity, while the latter does not. We also commonly omit the index $a,b$, so instead of $E_{a,b}(\mathbb{K})$ we write simply $E(\mathbb{K})$.
\end{remark}

The reason we introduced the point at infinity $\mathcal{O}$ is because it allows us to define the group binary operation $\oplus$ on the elliptic curve. The operation is sometimes called the \textbf{chord-tangent law}. Let us define it.

\vspace{-0.5em}

\begin{definition}\label{def:ec-add}
    Consider an elliptic curve $E(\mathbb{K})$ over any field $\mathbb{K}$. 
    We define $\mathcal{O}$ as the identity element of the group. 
    That is, for all points $P \in E(\mathbb{K})$, let $P \oplus \mathcal{O} = \mathcal{O} \oplus P = P$. 
    For any other non-identity elements $P=(x_P,y_P)$ and $Q=(x_Q,y_Q) \in E(\mathbb{K})$, define the $P\oplus Q = (x_R,y_R)$ as follows:
    \begin{enumerate}
        \item If $x_P\neq x_Q$, use the \textbf{chord method}. Define $\lambda := (y_P-y_Q)/(x_P-x_Q)$ --- the slope between $P$ and $Q$. Set the resultant coordinates as:
        \begin{equation*}
...
The aforementioned definition is illustrated in the Figure below\footnote{Illustration taken from \textit{``Pairing for Beginners''}~\cite[Figures 2.5 \& 2.6]{pairing-for-beginners}.}.
Further, we denote the group multiplication by $[\alpha]P$ for $\alpha \in \mathbb{Z}$ and $P \in E(\mathbb{K})$ and call it a \textit{scalar multiplication}.

\vspace{-2.5mm}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{contents/images/lecture_3/group_law.png}
    \label{fig:group_law}
\end{figure}
...
\begin{example}
    Consider an elliptic curve $E/\mathbb{Q}: y^2=x^3-2x$.
    Suppose we want to compute: \textit{(i)} $(-1,-1) \oplus (0,0)$ and \textit{(ii)} $[2](-1,-1)$ over $E/\mathbb{R}$.

    \begin{enumerate}
        \item[\textit{(i)}] One checks that the point $(2,2)$ lies on $E$ and moreover lies on the same line as $(-1,-1)$ and $(0,0)$.
    Consequently, $(-1,-1) \oplus (0,0) = (2,-2)$.
        \item[\textit{(ii)}] We compute the slope using \Cref{def:ec-add}: $\lambda = -\frac{1}{2}$. 
    Thus, the tangent line is $\ell': y = -\frac{1}{2}x+c$. Substituting $(-1,-1)$ into the equation, we get $c=-\frac{3}{2}$ and so the tangent line is $y=-\frac{1}{2}x-\frac{3}{2}$. The intersection of the curve and the line is $\left(\frac{9}{4},-\frac{21}{8}\right)$.
    Consequently, $[2](-1,-1) = \left(\frac{9}{4},\frac{21}{8}\right)$.
    \end{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.4\textwidth]{contents/images/lecture_3/ec_illustration_2.pdf}
        \caption{Illustration of the group law on the elliptic curve $E/\mathbb{R}: y^2=x^3-2x$. In \textcolor{oc-red-8}{red} we marked points lying on the line $\ell: y=x$. In \textcolor{oc-red-8}{dashed red}, we marked the line $\ell$, while in \textcolor{oc-green-7}{dashed green} --- the tangent line $\ell'$ at $(-1,-1)$, which is used to calculate $[2](-1,1)$.}
        \label{fig:ec_2}
    \end{figure}
\end{example}

\begin{theorem}
    $(E(\mathbb{K}),\oplus)$ forms an abelian group.
\end{theorem}

\begin{proof}
All properties besides associativity are obvious.
Checking associativity is a direct algebraic check, which we omit.
\end{proof}

Now, let us talk a bit about the group order. The group order is the number of elements in the group. For elliptic curves, the group order is typically denoted as $r$ or $n$. We are going to use $r$. 
The following theorem is the central tool for estimating the number of points on an elliptic curve.

\begin{theorem}[Hasse's Theorem]\label{th:hasse}
    Let $r := \#E(\mathbb{F}_{q})$ where $q=p^n$ for prime $p$ and $n \in \mathbb{N}$. Then,
    \begin{equation*}
        \left| r - (q+1) \right| \leq 2\sqrt{q}.
    \end{equation*}
\end{theorem}

A bit more intuitive explanation of above theorem: the number of points on an elliptic curve defined over the large finite field $\mathbb{F}_q$ is very close to $q$.

\begin{remark}
    $\#E(\mathbb{F}_q)$ can be computed in polylogarithmic time
    $O(\log^{\gamma}q)$ for some $\gamma>1$ (see, \textit{e.g.}, Schoof's algorithm~\cite{Schoof1995}). That being said, the number of
    points can be computed efficiently even for fairly large primes $q$.
\end{remark}

\begin{remark}
    Interestingly, \Cref{th:hasse} can be generalized for arbitrary curves.
    Specifically, given a curve $C(\mathbb{F}_q)$ of so-called \emph{genus} $g$ over the finite field $\mathbb{F}_q$, we have
    \begin{equation*}
        \left| \#C(\mathbb{F}_q) - (q+1) \right| \leq 2g\sqrt{q}.
    \end{equation*}
    Since elliptic curves are curves of genus $1$, \Cref{th:hasse} is the particular case of this fact.
    The definition of genus is well beyond the scope of this book.
\end{remark}

\subsection{Discrete Logarithm Problem on Elliptic Curves}
As defined in \Cref{section:security-analysis}, the \textbf{discrete logarithm}
problem on the elliptic curve is defined as follows: typically, $E(\mathbb{F}_p)$ is
cyclic, meaning there exist some point $G \in E(\mathbb{F}_p)$, called the
\textbf{generator}, such that $\langle G \rangle = E(\mathbb{F}_p)$. Given $P
\in E(\mathbb{F}_p)$, the problem consists in finding a scalar $\alpha \in
\mathbb{Z}_r$ such that $[\alpha]G = P$. 
...
Before delving into the projective coordinates and further zero-knowledge topics, let us first discuss the concept of relations, which will be intensively used from now on. Now, what is a relation? The definition is concise.

\begin{definition}
    Let $\mathcal{X},\mathcal{Y}$ be some sets. $\mathcal{R}$ is a \textbf{relation} if $\mathcal{R} \subseteq \mathcal{X} \times \mathcal{Y}$.
\end{definition}

Interpretation is approximately the following: suppose we have sets $\mathcal{X}$ and $\mathcal{Y}$. Then, relation $\mathcal{R}$ gives a set of pairs $(x,y)$, telling that elements $x \in \mathcal{X}$ and $y \in \mathcal{Y}$ are \textit{related}.

\begin{example}
    Define two sets: 
    \begin{align*}
        \mathcal{X} &:= \{\text{Oleksandr}, \text{Phat}, \text{Anton}\}, \\
        \mathcal{Y} &:= \{\text{Backend}, \text{Frontend}, \text{Research}\}.
    \end{align*}

    Define the following relation of ``person $x$ works in department $y$'':
    \begin{xequation*}
        \mathcal{R} = \{(\text{Oleksandr}, \text{Research}), (\text{Phat}, \text{Frontend}), (\text{Anton}, \text{Backend})\}.
    \end{xequation*}
    Obviously, $\mathcal{R} \subseteq \mathcal{X} \times \mathcal{Y}$, so $\mathcal{R}$ is a relation.
\end{example}
...
\begin{example}
    Let $E$ be a cyclic group of points on the Elliptic Curve of order $r \geq 2$ with a generator $\langle G \rangle = E$. Let $\mathcal{X} := \mathbb{Z}_r$ and $\mathcal{Y} := E$. Define a relation $\mathcal{R} \subseteq \mathcal{X} \times \mathcal{Y}$ by:
    \begin{xequation*}
...
\begin{remark}
    When we say that $\sim$ is a relation on a set $\mathcal{X}$, we mean that $\sim$ is a relation $\mathcal{R}$ on the following Cartesian product: $\mathcal{R} \subseteq \mathcal{X} \times \mathcal{X}$.
\end{remark}

Commonly $\sim$ resembles properties of equality.
Formally this is called an \textbf{equivalence relation}, which we define below.
...
\begin{example}
    Let $\mathcal{X}$ be the set of all people. Define a relation $\sim$ on $\mathcal{X}$ by $x \sim y$ iff $x,y \in \mathcal{X}$ have the same birthday. 
...
\begin{example}
    Let $\mathcal{X} = \mathbb{Z}$ and $n \in \mathbb{N}$. Define $\sim$ on $\mathcal{X}$ by $x \sim y$ iff $x \equiv y \pmod{n}$. Then the equivalence class of $x$ is the known to you set
    \begin{xequation*}
    ...
    \begin{align*}
        ...
        [1]_n &= \{\ldots,-2n+1,-n+1,1,n+1,2n+1,\ldots\}.    
    \end{align*}
    
\end{example}

Now, as we have said before, the set of all equivalence classes form a partition of the set $\mathcal{X}$. This means that any element $x \in \mathcal{X}$ belongs to exactly one equivalence class. This is a very important property, which we will use in the next section. Formally, we have the following lemma.
\begin{lemma}
...
\begin{definition}
    A \textbf{projective space}, denoted $\mathbb{P}^2(\mathbb{K})$, is a set $\mathbb{A}^3(\mathbb{K}) \setminus \{0\}$ modulo the following equivalence relation:
    $(X_1:Y_1:Z_1) \sim (X_2:Y_2:Z_2)$ iff there exists $\lambda \in \mathbb{K}^{\times}$
    such that $X_1=\lambda X_2, Y_1=\lambda Y_2, Z_1 = \lambda Z_2$.
\end{definition}

\begin{remark}
    Although we specify the definition for $n=2$, the definition can be easily generalized to any $\mathbb{P}^n(\mathbb{K})$.
\end{remark}
...
\begin{example}
    Consider the projective space $\mathbb{P}^2(\mathbb{R})$. Then, two points $(x_1,y_1,z_1),(x_2,y_2,z_2) \in \mathbb{R}^3$ are equivalent iff there exists a non-zero $\lambda \in \mathbb{R}^{\times}$ such that $(x_1,y_1,z_1) = (\lambda x_2, \lambda y_2, \lambda z_2)$. For example, $(1,2,3) \sim (2,4,6)$ since $(1,2,3) = 0.5 \cdot (2,4,6)$.
\end{example}
...
\begin{example}
    ...
    \begin{center}
        \scalebox{0.9}{
        \begin{tabular}{cc}
            \centering\includegraphics[trim={100 100 100 200}, width=0.47\linewidth, clip]{contents/images/lecture_4/line_1_view_1.pdf} &
            \centering\includegraphics[trim={100 100 100 200}, width=0.47\linewidth, clip]{contents/images/lecture_4/line_1_view_2.pdf}
        \end{tabular}
        }
        {\textbf{Illustration:} Geometric interpretation of $\mathbb{P}^2(\mathbb{R})$, the same scene from different perspectives. The \textcolor{oc-red-8}{\textbf{red}} line is represented by equation $(2t,3t,t)$, \textcolor{oc-blue-7}{\textbf{blue}} line by $(-2t,3t,3t)$, and \textcolor{green!60!black}{\textbf{green}} line is represented by $(t,-2t,5t)$ for parameter $t \in \mathbb{R}$.}
    \end{center}

    \vspace{-2mm}

    Here, the figure demonstrates three equivalence classes, being a set of points on the \textcolor{oc-red-8}{\textbf{red}}, \textcolor{oc-blue-7}{\textbf{blue}}, and \textcolor{green!60!black}{\textbf{green}} lines (except for the origin). 
    
    The reason why, geometrically, each equivalence class lies on a line that passes through the origin is as follows: suppose we have a point $\mathbf{v}_0 = (x_0,y_0,z_0) \in \mathbb{R}^3$, represented as a vector. Then, the set of all points that are equivalent to $(x_0,y_0,z_0)$ is the set of all points $(\lambda x_0, \lambda y_0, \lambda z_0) = \lambda\mathbf{v}_0$ for $\lambda \in \mathbb{R}\setminus \{0\}$. So $\mathbf{v}_0$ is the representative of equivalence class $[\mathbf{v}_0] = \{\lambda\mathbf{v}_0: \lambda \in \mathbb{R},\lambda \neq 0\}$. Now notice that this is a parametric equation of a line that passes through the origin and the point $\mathbf{v}_0$: for $\lambda=0$ (if we assume the expression is also defined for zero $\lambda$) we have the origin $\mathbf{0}$, while for $\lambda=1$ we have the point $\mathbf{v}_0$. Then, any other values of $\lambda$ within $[0,1]$ or outside it give points lying on that same line.
\end{example}
...
\begin{example}
    Again, consider three lines from the previous example. Now, we additionally draw a plane $\pi: z=1$ in the 3-dimensional space:

    \begin{center}
        \includegraphics[trim={100 100 100 200}, width=0.45\linewidth, clip]{contents/images/lecture_4/line_2.pdf}
        
        \small{\textbf{Illustration:} Geometric interpretation of converting projective to affine form.}
    \end{center}

    By using the map $(X:Y:Z) \mapsto (X/Z,Y/Z)$, all points on the line get mapped to the intersection of the line with the plane $\pi: z=1$. This way, for example, points on the \textcolor{oc-red-8}{\textbf{red line}} $\ell_{\text{red}}$ get mapped to the point $A'=(2,3,1)$, corresponding to $(2,3)$ in affine coordinates. So, for example, point $(6,9,3) \in \ell_{\text{red}}$, lying on the same line, gets mapped to $(6/3,9/3) = (2,3)$.

    Similarly, all \textcolor{oc-blue-7}{\textbf{blue line}} points get mapped to the point $B'=(-2/3,1,1)$, while all \textcolor{green!60!black}{\textbf{green line}} points get mapped to the point $C'=(0.2,-0.4,1)$\footnote{One can verify that based on the equations provided from the previous example.}.
\end{example}
...
Observe the following: all points $(0:\lambda:0)$ always lie on the curve $E_{\mathbb{P}}(\overline{\mathbb{F}}_p)$. Moreover, the map from the projective form to the affine form is ill-defined for such points, since we would need to divide by zero. So, we can naturally make the points $(0:\lambda:0)$ to be the set of points at infinity. This way, we can define the point at infinity as $\mathcal{O} = (0:1:0)$.

Finally, let us summarize what we have observed so far.

\begin{definition}
    The \textbf{homogeneous projective form of the elliptic curve} $E_{\mathbb{P}}(\overline{\mathbb{F}}_p)$ is defined as the set of all points $(X:Y:Z) \in \mathbb{P}^2(\overline{\mathbb{F}}_p)$ in the projective space that satisfy the equation
    \begin{xequation*}
...
\begin{example}
    ...
    \begin{center}
        \includegraphics[trim={275 100 225 100}, width=0.35\linewidth, clip]{contents/images/lecture_4/projective_ec.pdf}
        
        {\textbf{Illustration:} BN254 Curve Elliptic Curve in Projective Form over $\mathbb{R}$. In \textcolor{black!80}{\textbf{gray}} is the surface, while \textcolor{oc-red-8}{\textbf{red}} points are the points on the affine curve (lying on the plane $\pi: z=1$).}
    \end{center}
...
Hold on, but why did we use the term \textit{homogeneous}? The reason why is because we defined equivalence as follows: $(X:Y:Z) \sim (\lambda X: \lambda Y: \lambda Z)$ for some $\lambda \in \overline{\mathbb{K}}$, called \textbf{homogeneous coordinates}. However, this is not the only way to define equivalence. Consider a more general form of equivalence relation:
\begin{xequation*}
...
\begin{example}
    ...
    \begin{center}
        \includegraphics[trim={275 140 225 100}, width=0.3\linewidth, clip]{contents/images/lecture_4/projective_ec_jacobian.pdf}
        
        {\textbf{Illustration:} BN254 Curve Elliptic Curve in Jacobian Projective Form over $\mathbb{R}$. In \textcolor{black!80}{\textbf{gray}} is the surface, while \textcolor{oc-red-8}{\textbf{red}} points are the points on the affine curve (lying on the plane $\pi: z=1$).}
    \end{center}
...
Denote by $\textcolor{oc-blue-7}{\mathsf{M}}$ the cost of multiplication, by $\textcolor{green!60!black}{\mathsf{S}}$ the cost of squaring, and by $\textcolor{oc-red-8}{\mathsf{I}}$ the cost of inverse operation in $\mathbb{F}_p$. Note that we do not count addition/inverse costs as they are significantly lower than operations listed. Then, the cost of adding two points using above formula is $2\textcolor{oc-blue-7}{\mathsf{M}} + 1\textcolor{green!60!black}{\mathsf{S}} + 1\textcolor{oc-red-8}{\mathsf{I}}$. Indeed, our computation can proceed as follows:
\begin{enumerate}
    \item Calculate $t_1 \gets (x_Q-x_P)^{-1}$, costing $1\textcolor{oc-red-8}{\mathsf{I}}$.
    \item Calculate $\lambda \gets (y_Q-y_P)t_1$, costing $1\textcolor{oc-blue-7}{\mathsf{M}}$.
    ...
    \item Calculate $y_R \gets \lambda (x_P-x_R) - y_P$, costing $1\textcolor{oc-blue-7}{\mathsf{M}}$.
\end{enumerate}

Well, there are just 4 operations in total, so what can go wrong? The problem is that we need to calculate the inverse of $(x_Q-x_P)$, which is a very, very costly operation. In fact, typically $1\textcolor{oc-red-8}{\mathsf{I}} \gg 20\textcolor{oc-blue-7}{\mathsf{M}}$ or even worse, the ratio might reach $80$ in certain cases.

Now imagine we want to add 4 points, say $P_1 \oplus P_2 \oplus P_3 \oplus P_4$: this costs $6\textcolor{oc-blue-7}{\mathsf{M}} + 3\textcolor{green!60!black}{\mathsf{S}} + 3\textcolor{oc-red-8}{\mathsf{I}}$. Now we have 3 inverses, which is a lot. Finally, if we are to add much larger number of points (for example, when finding the scalar product), this gets even worse. 

Projective coordinates is a way to solve this problem. The idea is to represent points in the projective form $(X:Y:Z)$, so when adding two points in projective form, you still get a point in a form $(X:Y:Z)$. Then, after conducting a series of additions, you can convert the point back to the affine form. 

But why is adding two points, say, $(X_P:Y_P:Z_P)$ and $(X_Q:Y_Q:Z_Q)$, in the projective form more efficient? We will not derive the formulas, but trust us that they have the following form:
\begin{xequation*}
    \scalebox{0.9}{$
    \begin{aligned}
...
Do not be afraid, you do not need to understand how this formula is derived. But notice that despite the very scary look, there is no inversions involved! Moreover, this formula can be calculated in only $12\textcolor{oc-blue-7}{\mathsf{M}}+2\textcolor{green!60!black}{\mathsf{S}}$! So all in all, this is much more efficient than $2\textcolor{oc-blue-7}{\mathsf{M}} + 1\textcolor{green!60!black}{\mathsf{S}} + 1\textcolor{oc-red-8}{\mathsf{I}}$.
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item Convert affine points to projective form using $(x,y) \mapsto (x:y:1)$.
        \item Perform all operations in the projective form, which do not involve inversions.
        \item Convert the result back to affine form by $(X:Y:Z) \mapsto (X/Z,Y/Z)$.
    \end{enumerate}

    This is illustrated in the Figure below.
    \begin{center}
        \includegraphics[width=0.8\linewidth]{contents/images/lecture_4/strategy.pdf}
        
        {\textbf{Illustration:} General strategy when performing operations over Elliptic Curves.}
    \end{center}
\end{proposition}
...
First idea: let us simply add $P$ to itself $k$ times. However, the complexity would be $O(k)$ in this case, which is even harder than solving the discrete logarithm problem (recall that the discrete logarithm problem has a complexity of $O(\sqrt{k})$).
...
Good news: now we have a complexity of $O(N)=O(\log k)$, which is way much better than a linear one. In fact, many more optimized methods have the same assymptotic complexity (meaning, a logarithmic one), so it turns out that 
we cannot do much better than that. However, the main advantage of other, more optimized methods is that we can avoid making too many additions (here, in the worst case, we have to make $N$ additions), which is a costly operation (and more expensive than doubling).
For a more comprehensive overview, the reader is encouraged to read~\cite{guide-to-ec}.
...
Pairing is the core object used in threshold signatures, zk-SNARKs
constructions, and other cryptographic applications. 

Consider the \textit{Decisional Diffie-Hellman problem} which we described in \Cref{section:security-analysis} 
(based on $g^{\alpha},g^{\beta}$ and $g^{\gamma}$, decide whether $\gamma = \alpha\beta$). Turns out that for curves 
where the so-called \textit{embedding degree}\footnote{We will mention what that is later, but still this term is 
quite hard to define.} is small enough, this problem is easy to solve. This might sound like a significant security issue, but 
it turns out that although some information about the discrete logarithm is leaked, it is not enough to break the 
security of the system (\textit{e.g.}, solve the \textit{Computational Diffie-Hellman problem}). Pairings is the exact 
object that allows us to solve the Decisional DH problem.
...
just given $u=g^{\alpha},v=g^{\beta}$ we can check whether $\alpha\beta+5=0$ (which is problematic to check without 
having a pairing). 

\begin{definition}
    \textbf{Pairing} is a bilinear, non-degenerate, efficiently computable map $e: \textcolor{oc-red-8}{\mathbb{G}_1} \times \textcolor{oc-blue-7}{\mathbb{G}_2} \to \textcolor{oc-green-7}{\mathbb{G}_T}$, where $\textcolor{oc-red-8}{\mathbb{G}_1},\textcolor{oc-blue-7}{\mathbb{G}_2}$ are two groups (typically, elliptic curve groups) and $\textcolor{oc-green-7}{\mathbb{G}_T}$ is a target group (typically, a set of scalars). Let us decipher the definition:
    \begin{itemize}
        \item \textbf{Bilinearity} means the following:
        \begin{xequation*}
            e([a]\textcolor{oc-red-8}{P},[b]\textcolor{oc-blue-7}{Q}) = e([ab]\textcolor{oc-red-8}{P},\textcolor{oc-blue-7}{Q}) = e(\textcolor{oc-red-8}{P},[ab]\textcolor{oc-blue-7}{Q}) = e(\textcolor{oc-red-8}{P},\textcolor{oc-blue-7}{Q})^{ab}.        
        \end{xequation*}
        \item \textbf{Non-degeneracy} means that $e(G_1,G_2) \neq 1$ (where $G_1,G_2$ are generators of $\textcolor{oc-red-8}{\mathbb{G}_1},\textcolor{oc-blue-7}{\mathbb{G}_2}$, respectively). To put it more simply, this property says that the pairing is not trivial.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{contents/images/lecture_4/pairing.png}
...
\begin{example}
    ...
    is pairing. Indeed, it is bilinear. For example, $e(ax,by) = 2^{abxy} = (2^{xy})^{ab} = e(x,y)^{ab}$ or $e(ax,by)=2^{abxy}=2^{(x)(aby)}=e(x,aby)$. Moreover, it is non-degenerate, since $e(1,1)=2 \neq 1$. Finally, it is clearly efficiently computable.

    However, this is a quite a trivial example since working over integers is typically not secure. For example, the discrete logarithm over $\mathbb{Z}_r$ can be solved in subexponential time. For that reason, we want to build pairings over elliptic curves.
\end{example}
...
\begin{example}
    ...
    \begin{itemize}
        \item $\textcolor{oc-red-8}{\mathbb{G}_1}$ --- points on the regular curve $E(\mathbb{F}_p)$.
        \item $\textcolor{oc-blue-7}{\mathbb{G}_2}$ --- $r$-torsion points on the twisted curve $E'(\mathbb{F}_{p^2})$ over the field extension $\mathbb{F}_{p^2}$ (with equation $y^2 = x^3+\frac{3}{\xi}$ for $\xi=9+u \in \mathbb{F}_{p^2}$).
        \item $\textcolor{oc-green-7}{\mathbb{G}_T}$ --- $r$th roots of unity $\Omega_r \subseteq \mathbb{F}_{p^{12}}^{\times}$.
    \end{itemize}

    This definition is quite dense and even understanding the input and output parameters is quite hard. So let us decipher some components:
    \begin{itemize}
...
\begin{remark}
    One might a reasonable question: where does this $12$ come from? The answer is following: the so-called \textbf{embedding degree} of BN254 curve is $k=12$. This number is the key to understanding why we are working over such large extensions when calculating the pairing. The formal description is quite hard, but the intuition is following: the embedding degree is the smallest number $k$ such that all the $r$th roots of unity lie inside the extended field $\mathbb{F}_{p^k}$. If $k$ was smaller, the output of pairing would contain fewer than $r$ points and some points would be missing, which would make the pairing more trivial. For that reason, we need to have $\Omega_r \subseteq \mathbb{F}_{p^k}$.
\end{remark}
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item $k$ is the smallest positive integer such that $\mathbb{F}_{p^k}$ contains all of the $r$-th roots of unity in $\overline{\mathbb{F}}_p$, that is $\Omega_r \subseteq \mathbb{F}_{p^k}$.
        \item $k$ is the smallest positive integer such that $E(\overline{\mathbb{F}}_p)[r] \subseteq E(\mathbb{F}_{p^k})$
    \end{itemize}
\end{definition}

Notice the following fact: lower embedding degree is faster to work with, since it allows us to work over smaller fields. 
However, for arbitrary randomly chosen elliptic curves, this embedding degree is typically too large for practical applications. 
Consequently, one needs to craft elliptic curves specifically to have a small embedding degree. 
For example, a famous \texttt{secp256k1} curve has an embedding degree
\begin{equation*}
...
\begin{remark}
    One might ask why usually, when dealing with pairings, we do not get to work with field extensions that much in practice; for instance, if you were to write \texttt{groth16} from scratch using some mathematical libraries, you will not need to work with $\mathbb{F}_{p^{12}}$ arithmetic specifically. The reason is that mathematical libraries usually implement the following abstraction: given a set of points $\{(P_i,Q_i)\}_{i=1}^n \subseteq \mathbb{G}_1^n \times \mathbb{G}_2^n$, the function checks whether
    \begin{xequation*}
    ...
    \textbf{Interesting fact:} this condition is specified in the \texttt{ecpairing} precompile standard used in \textit{Ethereum}: see for example \url{https://www.evm.codes/precompiled?fork=osaka#0x08} for details.
\end{remark}

\subsubsection{Case Study: BLS Signature}

One of the most elegant applications of pairings is the BLS Signature scheme. 
Compared to ECDSA or other signature schemes, BLS can be formulated in three lines.
The main reason why it is attractive for practitioners is that it is succinct and enables efficient non-interactive aggregation.
...
\begin{definition}
    ...
    \begin{itemize}
        \item $\mathsf{Gen}(1^{\lambda})$: Key generation. $\mathsf{sk} \leftarrowS \mathbb{Z}_q, \mathsf{pk} \gets [\mathsf{sk}] G_2 \in \mathbb{G}_2$.
...
\begin{equation*}
    e(\sigma,G_2) = e(\textcolor{oc-blue-7}{[\mathsf{sk}]}\mathsf{H}(m),G_2) = e(\mathsf{H}(m), \textcolor{oc-blue-7}{[\mathsf{sk}]}G_2) = e(\mathsf{H}(m), \mathsf{pk})
\end{equation*}
...
\begin{example}
    Finally, let us prove something more interesting. For instance, based on
    $(x_1,x_2)$, whether
    \begin{xequation*}
        x_1^2 + x_1x_2 = x_2
    \end{xequation*}

    Suppose pairing $e: \mathbb{G} \times \mathbb{G} \to \mathbb{G}_T$ is
    symmetric. Alice can then calculate $P \gets [x_1]G, Q \gets [x_2]G \in
    \mathbb{G}$. Then, the condition can be verified by checking whether
    \begin{xequation*}
    ...
    \begin{xequation*}
        \begin{aligned}
            e(P, P\oplus Q)e(G, \ominus Q) &= e([x_1]G,[x_1+x_2]G)e(G,[x_2]G)^{-1} \\ 
                                         &= e(G,G)^{x_1(x_1+x_2)}e(G,G)^{-x_2} \\
                                         &= e(G,G)^{x_1^2+x_1x_2-x_2}
        \end{aligned}        
    \end{xequation*}
...
\exerciseTitle{Exercise 1.}{Suppose that elliptic curve is defined as $E/\mathbb{F}_{7}: y^2=x^3+b$. Suppose $(2,3)$ lies on the curve. What is the value of $b$?}

\begin{xexercise}
    {Exercise 2.}
    {Sum of which of the following pairs of points on the elliptic curve $E/\mathbb{F}_{11}$ is equal to the point at infinity $\mathcal{O}$ for any valid curve equation?}
    {2}
    {
        \item $P=(2,3),Q=(2,8)$.
        \item $P=(9,2),Q=(2,8)$.
...
\begin{xexercise}
    {Exercise 3.}
    {Consider an elliptic curve $E$ over $\mathbb{F}_{167^2}$. Denote by $r$ the order of the group of points on $E$ (that is, $r=|E|$). Which of the following \textbf{can} be the value of $r$?}
    {3}
    {
        \item $167^2 - 5$
        \item $167^2-1000$
...
\begin{xexercise}
    {Exercise 4.}
    {Suppose that for some elliptic curve $E$ the order is $|E| = qr$ where both $q$ and $r$ are prime numbers. Among listed, what is the most optimal complexity of algorithm to solve the discrete logarithm problem on $E$?}
    {3}
    {
        \item $O(qr)$
        \item $O(\sqrt{qr})$
...
\begin{xexercise}
    {Exercise 5.}
    {What is \textbf{not} a valid equivalence relation $\sim$ over a set $\mathcal{X}$?}
    {1}
    {
...
\begin{xexercise}
    {Exercise 6.}
    {Suppose that over $\mathbb{R}$ we define the following equivalence relation: $a \sim b$ iff $a-b \in \mathbb{Z}$ ($a,b \in \mathbb{R}$). What is the equivalence class of $1.4$ (that is, $[1.4]_{\sim}$)?}
    {1}
    {
...
\begin{xexercise}
    {Exercise 7.}
    {Which of the following pairs of points in homogeneous projective space $\mathbb{P}^2(\mathbb{R})$ are \textbf{not} equivalent?}
    {2}
    {
...
\begin{xexercise}
    {Exercise 8.}
    {The main reason for using projective coordinates in elliptic curve cryptography is:}
    {1}
    {
...
\begin{xexercise}
    {Exercise 9.}
    {Suppose $k=19$ is a scalar and we are calculating $[k]P$ using the double-and-add algorithm. How many elliptic curve point addition operations will be performed?}
    {5}
    {
        \item $0$.
        \item $1$.
...
\begin{xexercise}
    {Exercise 10.}
    {What is the minimal number of inversions needed to calculate the value of expression (over $\mathbb{F}_p$)
    \begin{equation*}
...
\begin{xexercise}
    {Exercise 11.}
    {Given pairing $e: \mathbb{G}_1 \times \mathbb{G}_2 \to \mathbb{G}_T$ with $G_1$ --- generator of $\mathbb{G}_1$ and $G_2 \in \mathbb{G}_2$ --- generator of $\mathbb{G}_2$, which of the following is \textbf{not} equal to $e([3]G_1, [5]G_2)$?}
    {2}
    {
        \item $e([5]G_1, [3]G_2)$.
        \item $e([4]G_1, [4]G_2)$.
