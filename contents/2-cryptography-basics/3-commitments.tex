...
\begin{definition}[Commitment Scheme]
    ...
    \begin{enumerate}
        ...
        \item $\mathsf{Setup} (1^{\lambda})$: returns public parameter $\mathsf{pp}$ for both committer and verifier;
...
\begin{definition}[Commitment Scheme properties]
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item Challenger chooses a random bit $b$, commits to both messages: 
                ...
                $c_0 \gets \mathsf{Commit}(pp, m_0), c_1 \gets \mathsf{Commit}(pp,m_1)$, 
        ...
        \item \textit{Binding: } prover could not find another message $m_1$ and open the commitment $c$ without revealing the committed message $m$.
...
\begin{enumerate}
    ...
    \item Prover samples random value $r$ (usually called blinding factor) from a challenge space $\mathcal{C} \subset \mathbb{Z}$:
...
However, due to the collision resistance, finding such $m'$ and $r'$ is computationally infeasible. This means the \textit{Prover}
cannot convince the \textit{Verifier} that the commitment was made to another value providing a \textit{binding} property.
...
all possible input values. Given a large challenge space, the probability of the \textit{Verifier} of finding $(m, r)$ such that
...
\begin{enumerate}
    \item Prover and Verifier agree on $G$ and $U$ points in an elliptic curve point group $\mathbb{G}$, $q$ is the order of the group.
...
Pedersen commitments have some advantages compared to hash-based commitments.
Additively homomorphic property allow us to accumulate multiple commitments into one.
...
Vector commitment schemes allow to commit to a vector of values rather than a value and a blinding term.
...
A naive approach for a vector commitment would be to hash the whole vector. A more sophisticated scheme uses divide-and-conquer approach by building a binary tree out of vector elements.
...
To prove the inclusion of an element in the tree, a corresponding Merkle Branch is used. In the example below, $M_1$ inclusion is proved, and 
...
Polynomial commitment can be used to prove that the committed polynomial satisfies certain properties $P(x_1, x_2, \ldots, x_n) = y$, without revealing what the polynomial is.
...
Suppose prover $\mathcal{P}$ wants to convince verifier $\mathcal{V}$ that he knows evaluation of polynomial $p(x) = a_d x^{d} + \dots + a_1 x + a_0 \in \mathbb{F}_{q}[x]$ at some $u \in \mathbb{F}_q$: $p(u) = y$ without revealing anything about $p(x)$. Let $\mathbb{G}$ be a cyclic additive group of order $q$ where \textit{discrete logarithm} problem is hard. $G, H \in \mathbb{G}$ -- points with unknown discrete logarithms between them.

\textbf{Simple polynomial commitment scheme}
\begin{enumerate}
    \item \textit{Commit to polynomial}. Prover selects random $\gamma_i \xleftarrow{R} \mathbb{F}_q$ for all $i \in [d]$, computes Pedersen commitments for each coefficient: $\forall i \in [d]: C_i = [a_i]G + [\gamma_i]H$ and sends them to Verifier.
...
Due to Schwartz-Zippel lemma, the probability that Prover can cheat and convince Verifier that he knows valid evaluation to not-commited polynomial is at most $\frac{1}{q}$, hence the scheme is sound.
...
\begin{enumerate}
    ...
    Polynomial $q(x)$ is called ``quotient polynomial`` and exists if and only if $p(x_0) = y_0$:
    \begin{enumerate}
    ...
    The existence of this quotient polynomial serves
    ...
    Polynomial commitment schemes such as KZG are used in zero-knowledge proof system to encode circuit constraints as a polynomial, 
    so that the verifier can check random points to ensure that the constraints are met.
