...
\begin{equation*}
    f(T) = 2T^2 - (3p+q)T + (p^2 + pq)
\end{equation*}

By equating $f(T)=0$ we get two solutions: $r_1 = p$ and $r_2=\frac{p+q}{2}$. One 
...
in $4T^2+7$ and verify that the result is never $0$ over $\mathbb{F}_5$.

\textbf{Exercise 3. Answer: A.} Note that $p(T)=a(T-1)(T-2)$. Since $p(0)=2$, 
we have $a=1$. Therefore, $p(x)=T^2-3T+2$ and thus $b=-3$, $c=2$. The sum 
$a+b+c$ is then $0$.

\textbf{Exercise 4. Answer: C.} Roots of $q$ are $\pm i$. If $q \mid p_n$,
we must have $p_n(\pm i) = 0$. Thus we have $p_n(i)=i^{2n}+i^n-2=(-1)^n+i^n-2$. 
Consider four cases:
\begin{itemize}
    \item $n=4k$. Then, $p_n(i)=(-1)^{4k}+i^{4k}-2=0$.
    \item $n=4k+1$. Then, $p_n(i)=(-1)^{4k+1}+i^{4k+1}-2=-3+i \neq 0$.
    \item $n=4k+2$. Then, $p_n(i)=(-1)^{4k+2}+i^{4k+2}-2=-2 \neq 0$.
    \item $n=4k+3$. Then, $p_n(i)=(-1)^{4k+3}+i^{4k+3}-2=-3-i \neq 0$.
\end{itemize}

Therefore, $n$ must be divisible by $4$. It is easy to check that in such 
case $p_n(-i)$ equals $0$ as well.
...
$\text{Pr}[f(\bm{\alpha})=0 \,|\, \bm{\alpha} \leftarrowS S] \leq \deg
f/\#S$, so we need to find the size $\#S$. Our claim is that 
the number of solutions to $\alpha_1+\dots+\alpha_n=1$ over $\mathbb{F}_p$ is exactly 
$p^{n-1}$. Indeed, for any $p^{n-1}$ possible values of $(\alpha_2,\dots,\alpha_n)$ there 
is a unique $\alpha_1$ such that $\alpha_1+\dots+\alpha_n=1$, which is $1-\sum_{i=2}^n\alpha_i$. 
Therefore, $\text{Pr}[f(\bm{\alpha})=0] \leq \deg f/p^{n-1}$.
...
\begin{equation*}
    \|T\|^2 = \langle T, T\rangle = \int_0^1 \tau^2\,d\tau = \frac{1}{3},
\end{equation*}
so $\|T\| = 1/\sqrt{3}$.

\textbf{Exercise 6.} Let $q(T) = \alpha T + \beta$. Then
\begin{equation*}
    \langle q, T^2\rangle = \int_0^1 (\alpha \tau+\beta)\tau^2\,d\tau = \int_0^1 (\alpha \tau^3 + \beta \tau^2)\,d\tau = \frac{\alpha}{4}+\frac{\beta}{3}.
\end{equation*}
...
\begin{equation*}
    q(T) = 4T-3,
\end{equation*}
and one checks directly that $\int_0^1 (4\tau-3)\tau^2\,d\tau = \left[\tau^4-\tau^3\right]_0^1 = 0$, as required.
...
\textbf{Exercise 1. Answer: C.} We have $\mathsf{H}(x)=c$. By the definition of $\mathsf{H}$,
we have $13x+17=39$ over $\mathbb{F}_{41}$. Therefore, $13x=22$,
so $x=22 \cdot 13^{-1} = 8$. 

\textbf{Exercise 2. Answer: A.} If $h=g^6$ then the Pedersen commitment for element
$(m,r)$ can be written as $\mathsf{com}(m,r) = g^mh^r=g^mg^{6r}=g^{m+6r}$.
Therefore, to fool Dmytro, if suffices to take such $(m',r')$ that
$m'+6r'=m+6r$. Substituting numbers we get $15+6r'=3+6\cdot 7=45$.
Therefore, $6r'=30$ so finally $r'=5$.

\textbf{Exercise 3. Answer: D.} It suffices to manually check every option. For
option (D) we indeed have $\mathsf{H}(\mathsf{H}(17, \mathsf{H}(3,4)), 13) = 37$.  

\textbf{Exercise 4. Answer: C.} The quotient polynomial $g$ must satisfy
$g(T)\cdot (T-2)=f(T)$. It is easy to check that for $g(T)=T^2-8T+15$ we indeed
have $(T^2-8T+15)(T-2) = T^3-10T^2+31T-30$.
