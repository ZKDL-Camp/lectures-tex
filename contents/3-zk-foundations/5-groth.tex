...
Now, suppose during the setup phase, we have a trusted party that generated a random value $\tau \leftarrowS \mathbb{F}$ and public parameters $g^{\tau},g^{\tau^2},\dots,g^{\tau^{d}}$ for $d=2|\Circ|$ --- maximum degree of used polynomials (later we will use notation $\{g^{\tau^i}\}_{i \in [d]}$ for brevity). The party then \textbf{deletes} $\tau$. This way, we can now find the KZG commitment for each polynomial. Indeed, for example,
\begin{equation*}
    \mathsf{com}(\ell) \triangleq g^{\ell(\tau)} = g^{\sum_{i=0}^d \ell_i \tau^i} = \prod_{i=0}^d (g^{\tau^i})^{\ell_i},
\end{equation*}

and the same goes for $g^{r(\tau)}, g^{o(\tau)}, g^{h(\tau)}, g^{z(\tau)}$. Now, with these points, how can we verify that the polynomial $m(T) = \ell(T)r(T) - o(T)$ is correct? Well, first notice that the check is equivalent to
\begin{equation*}
...
\begin{remark}
    ...
    \begin{flalign*}
        ...
        \Leftrightarrow \quad & \ell(T)r(T) \equiv z(T)h(T) + o(T)
    \end{flalign*}
\end{remark}
...
\begin{tcolorbox}[title=Attempt \#1: Non-sound SNARK Protocol,
    ...
    Suppose we are given a circuit $\Circ$ with a maximum degree $d$ of polynomials used underneath. Thus, all parties additionally know the target polynomial $z(T)$.
    ...
    \begin{itemize}[label=\ding{51}]
        ...
        solution witness $\mathbf{z}$, and calculates the polynomials $\ell(T)$,
        $r(T)$, $o(T)$ through Lagrange Interpolation.
        \item Calculates $h(T) \gets (\ell(T)r(T) - o(T))\big/ z(T)$.
...
The informal reason why this makes KZG commitment sound is the following: suppose we have an adversary prover $\mathcal{P}^*$ that published commitment $c$ without knowing underlying polynomial $f(T)$. Now, there is no way for him to cheat the verifier. 
Indeed, what $\mathcal{P}^*$ needs to do is calculating $c^{\alpha}$, but he does not know $\alpha$ since, similarly to $\tau$, it was deleted as a part of the toxic waste. Besides, he cannot obtain $\alpha$ for the same reason he cannot obtain $\tau$.

For that reason, we modify the SNARK protocol to include not only commitments to polynomials but also PoE for each of them. Let us see what it looks like.
...
\begin{tcolorbox}[title=Attempt \#2: SNARK with PoE included,
    ...
    The prover $\mathcal{P}$ conducts the following steps:
    \begin{itemize}[label=\ding{51}]
        \item Runs the circuit with the statement $\mathbbm{x}$ and witness $\mathbbm{w}$, obtains the intermediate constraint values, and calculates the polynomials $\ell(T), r(T), o(T)$ through Lagrange Interpolation.
        \item Calculates $h(T) \gets (\ell(T)r(T) - o(T))\big/ z(T)$.
...
\begin{equation*}
    \ell(T) = \sum_{i=0}^n z_i\ell_i(T), \quad r(T) = \sum_{i=0}^n z_i r_i(T), \quad o(T) = \sum_{i=0}^n z_io_i(T).
\end{equation*}
while $m(T) = \ell(T)r(T) - o(T)$ is only known to the prover $\mathcal{P}$ since it contains the extended witness ($\mathbf{z}$) coefficients. However, the set of polynomials
\begin{equation*}
    \{\ell_i(T)\}_{i \in [n]}, \{r_i(T)\}_{i \in [n]}, \{o_i(T)\}_{i \in [n]}
\end{equation*}
...
How? Suppose the prover $\mathcal{P}$ knows the extended witness $\mathbf{z}$. Consider the polynomial $\ell(T) = \sum_{i=0}^n z_i\ell_i(T)$. $\mathcal{P}$ can compute the KZG commitment $\pi_L$ and its PoE $\pi_L'$ as follows:
\begin{equation*}
    \begin{aligned}
...
\textbf{Introducing new term with $\beta$}. To prove that the same $\mathbf{z}$ is used in all commitments, we need some ``checksum'' term that will somehow combine all polynomials $\ell(T)$, $r(T)$, and $o(T)$ with the witness $\mathbf{z}$. Moreover, we will need to compare this term with proofs $\pi_L$, $\pi_R$, and $\pi_O$. The best candidate for this is the following group element for some other random $\beta \leftarrowS \mathbb{F}$:
\begin{equation*}
    \begin{aligned}
...
\textbf{Introducing separate $\beta_L$, $\beta_R$, $\beta_O$.} Our proposal is to make $\beta$ different for each polynomial $\ell(T)$, $r(T)$, $o(T)$, making it much harder for the adversary to find inconsistent witnesses. That being said, during the setup phase, we choose arbitrary $\beta_L,\beta_R,\beta_O \leftarrowS \mathbb{F}$ and publish $\{g^{\beta_L\ell_i(\tau)+\beta_Rr_i(\tau)+\beta_Oo_i(\tau)}\}_{i \in [n]}$ as a part of the prover parameters $\mathsf{pp}$. Then, the prover $\mathcal{P}$ calculates the following additional commitment:
\begin{equation*}
...
\begin{tcolorbox}[title=Attempt \#3: Sound SNARK Protocol,
    ...
    The prover $\mathcal{P}$ conducts the following steps:
    \begin{itemize}[label=\ding{51}]
        \item Runs the circuit to get $\mathbf{z}$ and $\ell(T),r(T),o(T)$.
        \item Calculates $h(T) \gets (\ell(T)r(T) - o(T))\big/ z(T)$.
        \item Calculates the sound KZG commitments as follows: 
        \begin{align*}
...
$\mathbbm{x}$ and witness $\mathbf{z}$. According to the Circuit Satisfiability
...
(\widetilde{z}_1,\dots,\widetilde{z}_n)$ we pick out the set of indices
...
\begin{tcolorbox}[title=Attempt \#4: Sound SNARK for Public/Private Inputs,
    ...
    Suppose we are given a circuit $\Circ$ with a maximum degree $d$ of polynomials used underneath. Thus, all parties additionally know the target polynomial $z(T)$. \textcolor{green!50!black}{Additionally, we know that $\mathcal{I}_{\text{io}}$ corresponds to public signals, while $\mathcal{I}_{\text{mid}}$ corresponds to private signals.}
    ...
    \begin{itemize}[label=\ding{51}]
        \item Runs the circuit to get $\mathbf{z}$ and $\ell(T),r(T),o(T)$.
        \item Calculates $h(T) \gets (\ell(T)r(T) - o(T))\big/ z(T)$.
        \item \textcolor{green!50!black}{Splits $\ell(T) = \ell_{\text{mid}}(T) + \ell_{\text{io}}(T)$ --- intermediate and input/output parts. That being said, $\ell_{\text{mid}}(T) = \sum_{i \in \mathcal{I}_{\text{mid}}}\ell_i(T)$. Repeat for $r(T)$ and $o(T)$.}
...
for randomly selected $\delta_L, \delta_R, \delta_O \leftarrowS \mathbb{Z}_q$. Good, we concealed all the information about the witness $\mathbbm{w}$ in the commitment. However, we need to make sure that the verifier $\mathcal{V}$ can still verify the proof, but without modifying the verification mechanism itself. Notice that PoE verifications are still valid, but we need to modify something to make polynomial equality test hold. This can be done by perturbating the polynomial $h(T)$ by some (currently) unknown value $\Delta_H$. Let us derive it from the polynomial equality test:
\begin{equation*}
    (\ell(T) + \delta_L z(T))(r(T) + \delta_R z(T)) = (h(T) + \Delta_H)z(T) + (o(T) + \delta_O z(T)), \\
\end{equation*}

which, by expanding, gives us the following equation:
\begin{equation*}
    \begin{aligned}
        &\cancel{\ell(T)r(T)} + \delta_R \ell(T)z(T) + \delta_L z(T)r(T) + \delta_L \delta_R z(T)^2 = \\ 
        &= \cancel{h(T)z(T) + o(T)} + \Delta_Hz(T) + \delta_O z(T), \\
    \end{aligned}
\end{equation*}

where we can cancel out $\ell(T)r(T)$ and $h(T)z(T) + o(T)$ terms since they are equal based on initial construction. This way, we get the following expression for $\Delta_H$:
\begin{equation*}
    \Delta_H = \delta_O + \delta_R \ell(T) + \delta_L r(T) + \delta_L \delta_R z(T)
\end{equation*}
...
\begin{tcolorbox}[title=Attempt \#5: Turning SNARK into zk-SNARK,
    ...
    colupper=blue!75!gray} ]
    \small
    Suppose we are given a circuit $\Circ$ with a maximum degree $d$ of polynomials used underneath. Thus, all parties additionally know the target polynomial $z(T)$. Additionally, we know that $\mathcal{I}_{\text{io}}$ corresponds to public signals, while $\mathcal{I}_{\text{mid}}$ corresponds to private signals.
    ...
    \begin{itemize}[label=\ding{51}]
        \item Runs the circuit to get $\mathbf{z}$ and $\ell(T),r(T),o(T)$.
        \item Calculates $h(T) \gets (\ell(T)r(T) - o(T))\big/ z(T)$.
        \item Splits $\ell(T) = \ell_{\text{mid}}(T) + \ell_{\text{io}}(T)$ --- intermediate and input/output parts. That being said, $\ell_{\text{mid}}(T) = \sum_{i \in \mathcal{I}_{\text{mid}}}\ell_i(T)$. Repeat for $r(T)$ and $o(T)$.
...
While our built protocol is zk-SNARK, it is still not optimized for practical use. In fact, let us recap what the complexity of our protocol is.
...
Now, this is not bad at all! In fact, this is already practical for many applications. However, we can do better by a more clever choice of constants and terms. This is exactly what is done by Bryan Parno and Craig Gentry in their paper ``Pinocchio: Nearly Practical Verifiable Computation''~\cite{pinocchio}.
...
choose different generators for $\ell(T)$, $r(T)$, and $o(T)$. 
...
The next change is that we use different $\alpha$ values for $\ell(T)$, $r(T)$, and $o(T)$. Namely, we define $\alpha_L,\alpha_R,\alpha_O \leftarrowS \mathbb{F}$ and use them in the commitments. This way, in the setup phase, we additionally prepare the following values:
\begin{equation*}
...
\textcolor{green!50!black}{\textbf{Proving.}} Again, suppose intermediate polynomials for indices $\mathcal{I}_{\text{mid}}$ are $\ell_{\text{mid}}(T) = \sum_{i \in \mathcal{I}_{\text{mid}}}z_i\ell_i(T)$ and similarly for $r_{\text{mid}}(T)$ and $o_{\text{mid}}(T)$. The prover $\mathcal{P}$ calculates the following commitments:
...
\begin{tcolorbox}[title=Pinocchio Protocol,
    ...
    \begin{itemize}[label=\ding{51}]
        \item Runs the circuit to get $\mathbf{z}$ and $\ell(T),r(T),o(T)$.
        \item Calculates $h(T) \gets (\ell(T)r(T) - o(T))\big/ z(T)$.
        \item Splits $\ell(T) = \ell_{\text{mid}}(T) + \ell_{\text{io}}(T)$ --- intermediate and input/output parts. Repeat for $r(T)$ and $o(T)$.
...
Finally, Groth16 \cite{groth16} allows to reduce the number of pairings \textbf{down to 3}! This is done through a technique called \textbf{Generic Group Model} (GGM for short). Simply put, GGM allows the adversary to only make oracle requests to compute the group operations. For example, having a set $\{g^{\alpha r_i(\tau)}\}_{i \in [d]}$, adversary can compute only linear combinations of these values. In the particular case of Groth16, instead of considering $\ell_i(T)$, $r_i(T)$, and $o_i(T)$ separately, we construct their linear combinations as $Q_i(T) := \beta \ell_i(T) + \alpha r_i(T) + o_i(T)$, where $\alpha$ and $\beta$ are toxic parameters.
...
This section was greatly inspired by \cite{petkus2019}
