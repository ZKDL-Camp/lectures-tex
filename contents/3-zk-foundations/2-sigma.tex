...
Let us formalize it using theory from \Cref{section:relations_and_languages}. Suppose $\mathbb{G}$ is a cyclic group of order $q$ with a generator $g$. Then, the relation and language being considered:
\begin{equation*}
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item $\mathcal{V}$ sends a random challenge $e \leftarrowS \mathbb{Z}_q$ to $\mathcal{P}$.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Extractor rewinds back to the verifier's challenge step, generates a new challenge $e' \leftarrowS \mathbb{Z}_q$ and gets new prover's response $\sigma'$ (for the same prover's randomness $r$).
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item $\mathsf{Sign}(m,\mathsf{sk})$: The signer computes $r \gets \mathbb{Z}_q^{\times}$, $a \gets g^{r}$, $e \gets \mathsf{H}(m, a)$, $\sigma \gets r + \alpha e$ and outputs the signature $(a,\sigma)$.
        \item $\mathsf{Verify}((a, \sigma), m,\mathsf{pk})$: Verifier checks $g^{\sigma} = a \cdot u^e$ for $e \gets \mathsf{H}(m, a)$.
    \end{itemize}
\end{definition}

\begin{remark}
    Typically, one also uses a so-called ``\textit{key-prefixed}'' variant of the scheme, where the challenge $e$ is computed as $e \gets \mathsf{H}(\mathsf{pk}, m, a)$ for a random oracle $\mathsf{H}: \mathbb{G} \times \mathcal{M} \times \mathbb{G} \to \mathbb{Z}_q$. It was argued that such variant has a better multi-user security bound than the classical one.
\end{remark}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{contents/images/lecture_7/sigma_protocol_illustration.pdf}
...
\begin{definition}[Okamoto's Identification Protocol]
    ...
    \begin{enumerate}
        \item $\mathcal{P}$ computes $\alpha_r \leftarrowS \mathbb{Z}_q, \; \beta_r \leftarrowS \mathbb{Z}_q, \; u_r \gets g^{\alpha_r}h^{\beta_r}$ and sends commitment $u_r$ to $\mathcal{V}$.
        \item $\mathcal{V}$ samples the challenge $c \leftarrowS \mathbb{Z}_q$ and sends $c$ to $\mathcal{P}$.
...
\begin{definition}[Chaum-Pedersen Protocol]
    ...
    \begin{enumerate}
        \item $\mathcal{P}$ computes $\beta_r \leftarrowS \mathbb{Z}_q, \; v_r \gets g^{\beta_r}, \; w_r \gets u^{\beta_r}$ and sends commitment $(u_r,w_r)$ to $\mathcal{V}$.
        \item $\mathcal{V}$ samples the challenge $c \leftarrowS \mathbb{Z}_q$ and sends $c$ to $\mathcal{P}$.
...
\begin{definition}[Sigma Protocol for the pre-image of a homomorphism]
    ...
    \begin{enumerate}
        \item $\mathcal{P}$ computes $h_r \leftarrowS \mathbb{H}, t_r \gets \psi(h_r) \in \mathbb{T}$ and sends $t_r$ to the verifier $\mathcal{V}$.
        \item $\mathcal{V}$ samples the challenge $c \leftarrowS \mathcal{C} \subset \mathbb{Z}$ from the challenge space and sends $c$ to $\mathcal{P}$.
...
\begin{definition}[The AND Sigma Protocol]
    ...
    \begin{enumerate}
        ...
        \item The verifier computes the challenge $c \leftarrowS \mathcal{C}$ and sends it to $\mathcal{P}$.
...
\begin{definition}[The OR Sigma Protocol]
    ...
    \begin{enumerate}
        \item The prover chooses a random challenge $c_{b^*} \leftarrowS \mathcal{C}$ and generates random commitment and response $(a_{b^*}, z_{b^*})$ that form a valid accepting conversation $(a_{b^*}, c_{b^*}, z_{b^*})$ (essentially, the prover runs the simulator $(a_{b^*}, z_{b^*}) \leftarrowS \mathsf{Sim}_{b^*}(x_{b^*}, c_{b^*})$). Then, $\mathcal{P}$ also runs $\mathcal{P}_b(x_b, w)$ to get a valid commitment $a_b$ and sends $(a_0,a_1)$ to $\mathcal{V}$.
        \item The verifier sends a random challenge $c \leftarrowS \mathcal{C} \subseteq \{0,1\}^{\ell}$.
...
Now, let us implement the Okamoto Protocol from \Cref{section:okamoto_protocol} in SageMath! We propose to make the proof 
...
two random scalars $\alpha_r, \beta_r \leftarrowS \mathbb{Z}_q$ and computing
...
parameter $u$ to get the challenge $c$: $c \gets \mathsf{H}(u,u_r)$. The only 
caveat, though, is that we need to implement the hash function 
$\mathsf{H}: \mathbb{G} \times \mathbb{G} \to \mathbb{Z}_q$. For simplicity, let us 
take the hash function that concatenates the $x$-coordinates of the points
and hashes the result: $\mathsf{H}(u,u_r) = \mathsf{SHA256}(u_x \parallel (u_r)_x)$:
\begin{minted}{python}
...
restore the challenge $c \gets \mathsf{H}(u,u_r)$ from $u$ and $u_r$ values, specified in
...
\begin{exbox}{Exercises 1-5. In search of correct Schnorr's Identification Protocol\ldots}
    ...
    \textcolor{oc-pink-7}{\textbf{Attempt 1.}} Prover sends witness $\alpha$ to the verifier. Verifier checks whether $h = g^{\alpha}$.

    \textcolor{oc-pink-7}{\textbf{Attempt 2.}} Prover chooses random $r \leftarrowS \mathbb{Z}_q$ and sends $a \gets \alpha + r$ to the verifier. Verifier checks whether $h = g^a$.

    \textcolor{oc-pink-7}{\textbf{Attempt 3.}} Prover chooses random $r \leftarrowS \mathbb{Z}_q$, calculates $a \gets \alpha + r$ and sends both $(a,r)$ to the verifier. Verifier checks whether $g^r h = g^a$.

    \textcolor{oc-pink-7}{\textbf{Attempt 4.}} Prover chooses random $r \leftarrowS \mathbb{Z}_q$, calculates $a \gets g^r, z \gets \alpha+r$ and sends $(a,z)$ to the verifier. Verifier checks whether $a \cdot h = g^z$.

    \textcolor{oc-pink-7}{\textbf{Attempt 5.}} Prover chooses random $r \leftarrowS \mathbb{Z}_q$, calculates $a \gets g^r$, and sends $a$ to the verifier. Verifier chooses $e \leftarrowS \mathbb{Z}_q$ and sends to the prover. Prover calculates $z \gets \alpha e + r$ and sends to the prover. Verifier checks whether $a \cdot h^e = g^z$.
...
\begin{exbox}{Exercises 6-10. Non-Interactive Chaum-Pedersen Protocol.}
...
\begin{xexercise}
    ...
    {2}
    {
        \item $\mathsf{H}: \mathbb{G}^3 \times \mathbb{G}^2 \to \mathcal{C}$
        \item $\mathsf{H}: \mathbb{G}^3 \times (\mathbb{G} \times \mathbb{Z}_q) \to \mathcal{C}$
        \item $\mathsf{H}: \mathbb{G}^3 \to \mathcal{C}$
        \item $\mathsf{H}: \mathbb{G}^3 \times \mathbb{Z}_q \to \mathcal{C}$
    }
\end{xexercise}
...
\begin{xexercise}
    ...
    {2}
    {
        \item $\mathsf{H}((u,v,w),(v_r,w_r))$
        \item $\mathsf{H}((u,v,w),(v_r,\beta_r))$
        \item $\mathsf{H}(u,v,w)$
        \item $\mathsf{H}((u,v,w),\beta_r)$
