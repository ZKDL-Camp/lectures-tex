...
Now, let us see how these appear in practice.
...
component main = Math();
    ...
    (that can be computed out of circuit) and then assert that $x \times y = 1$
...
Next, let us apply the learned concepts to more complex examples. We start
...
Now, let us compare the mathematical and Circom representations.

% FIXME (@ZamDimon): Spacing below is freaking awful. Please fix it later.
\newsavebox{\circomcode}

% --- Fill the savebox with the lstlisting content BEFORE the tikzpicture ---
\begin{lrbox}{\circomcode}
\begin{minipage}{6cm} % Use a minipage for better width control
    \centering
    \scalebox{0.8}{\textbf{Circom Representation:}} \\[1ex] % Add a little space
    \begin{lstlisting}[language=Circom, numbers=none, escapeinside={<@}{@>}]
...
% --- Now, you can safely draw the tikzpicture ---
\begin{center}
    \vspace{-1em}
    \begin{tikzpicture}
        % This node is fine as it is
        ...
        % This node now uses the pre-rendered box, avoiding the error
        \node[inner sep=0pt] (circomside) at (2.0,0) {
            \usebox{\circomcode}
        };
    \end{tikzpicture}
    \vspace{-2.5em}
...
Now, let us take a closer look at how the witness is computed. In \Cref{section:circuits}, we had:
\[ \mathbf{z} = (1, r, x_1, x_2, x_3, \mathsf{mult}, \mathsf{selectMult}) \]

Given the inputs: $x_1 = 1, x_2 = 3, x_3 = 4$, we can quickly do the math
and find out that the actual witness should look like this: $\mathbf{z} =
...
The \texttt{.sym} file not only lists the signal names, but also includes information about the optimized signals.
...
Now, let us examine what the third column in the \texttt{.sym} file means.
...
\begin{remark}
    At the time of writing, \texttt{snarkjs} does not support any custom gates (as stated in their documentation).
...
This keyword will be useful in structures as the one below:
...
However, in small circuits or wherever the computation is inherently sequential (i.e., where the output of one part is the input to another), \texttt{parallel} has no effect.
...
\begin{remark}[On representing $\mathbb{F}_{p^{12}}$ element]
    One might wonder: why the element from $\mathbb{F}_{p^{12}}$ is represented as a pair of two arrays,
    ...
    consists of a triplet of $\mathbb{F}_{p^2}$ elements. For more details,~see~\Cref{section:finite-fields}.
\end{remark}
