...
However, in cryptography, we will not work with arbitrary multivariate
...
\begin{example}\label{example:mle}
    ...
    \vspace{-7.5px}
    \begin{gather*}
        \mathsf{eq}(X_1,X_2;(0,0)) = (1-X_1)(1-X_2), \;
        \mathsf{eq}(X_1,X_2;(0,1)) = (1-X_1)X_2, \\
        \mathsf{eq}(X_1,X_2;(1,0)) = X_1(1-X_2), \;
        \mathsf{eq}(X_1,X_2;(1,1)) = X_1X_2.
    \end{gather*}
    The multilinear extension $\widetilde{f}(X_1,X_2)$ is thus computed as:
    \vspace{-7.5px}
    \begin{align*}
        \widetilde{f}(X_1,X_2) &= 3(1-X_1)(1-X_2) + 4(1-X_1)X_2 + X_1(1-X_2) + 2X_1X_2 \\
        &= 3 - 2X_1 + X_2.
    \end{align*}
\end{example}
...
\textbf{Why cannot the verifier just compute the sum?} Typically, $v$ is a fairly
...
def boolean_hypercube_sum(f: PolynomialRing) -> Fp:
    """
    Computes the sum of the polynomial f over the hypercube {0,1}^v.
    The boolean hypercube is represented by evaluating the polynomial
    at all combinations of 0 and 1 for each variable.
    """
...
\begin{align*}
    ...
    &+ 850030718X_1^2X_4X_6^3X_7^2X_8X_9 + 560601732X_0X_3X_5X_6X_7X_8X_9^4 \\
    &- 390725845X_0^2X_2^3X_3X_9^3 \\
...
def fiat_shamir_sample(input_string: str) -> Fp:
    """
    Samples a random value from the finite field GF(p) 
...
class SumCheckProtocol:
    ...
    def __init__(
        ...
        """
        Initializes the SumCheck protocol with a polynomial
        and the claimed sum.

        Args:
            polynomial (PolynomialRing): The polynomial 
            to be checked.
            claimed_sum (Fp): The sum claimed by 
            the prover over the boolean hypercube.
        """
    ...
    def fiat_shamir_from_polynomials(polynomials: list[PolynomialRing]) -> Fp:
        ...
        Args:
            polynomials (list[PolynomialRing]): 
            List of polynomials to be used for sampling.
    ...
    def prove(self) -> dict:
        """
        Prover generates the transcript 
        of the polynomial and the claimed sum.

        Returns:
            dict: A transcript containing the polynomial, 
            claimed sum, random values, and polynomials.
        """
        ...
        for j in range(v):
            ...
            # Finding polynomial s_j that represents 
            ...
            for i in range(2**(v-j-1)):
                # Convert i to a binary representation 
                # of length v-j-1
                binary_representation = [(i >> k) & 1 for k in range(v-j-1)]
                # Evaluate the polynomial at this point
                point_value = self.f(*transcript['random_values'][:j], 
                                      variables[j], *binary_representation)
    ...
    def verify(self, transcript: dict) -> bool:
        ...
        # Assert that random_values is formed 
        # correctly using Fiat-Shamir heuristic
        if len(random_values) != v:
            print("Invalid number of random values in the transcript.")
            return False
        for i in range(v):
            if random_values[i] != self.fiat_shamir_from_polynomials(
                polynomials[:i+1]
            ):
                print(f"Random value at index {i} does 
                        not match the Fiat-Shamir sample.")
                return False

        # Assert that each s_r(0) + s_r(1) 
        # matches the claimed sum H
        for j in range(v):
            # During the first round, simply check 
            # that s_1(0) + s_1(1) = H
            if j == 0:
                s_1 = polynomials[0]
                if s_1(0) + s_1(1) != H:
            ...
            # For subsequent rounds, check that the 
            # sum of the polynomials matches the claimed sum
        ...
        if last_polynomial(last_random) != f(*random_values):
            print(f"Final check failed: 
                  {last_polynomial(last_random)} != {f(*random_values)}")
            return False
...
$\widetilde{\mathtt{C}}: \mathbb{F}^{\ell} \to \mathbb{F}$). Here is how it goes:
\begin{itemize}
...
protocol can help to keep the same asymptotics, but does not reveal 
