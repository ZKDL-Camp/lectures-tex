...
A \textbf{set} is a well-defined collection of objects. The objects that belong
to a set are called its \textbf{elements}. We will denote sets by capital
letters, if $a$ is an element of the set $A$, we write $a \in A$.
...
\begin{itemize}
    ...
    \item $\mathbb{C}$ -- a set of complex numbers\footnote{Complex number is, simply put, a formal expression $x+iy$ for $i^2=-1$ and $x,y \in \mathbb{R}$.}. 
    Examples: $1+2i, 5i, -7-5.7i, \dots$.
\end{itemize}

To represent the number of elements in a set $A$, we either write $|A|$ or $\#A$. If the set is finite, $\#A \in \mathbb{N}$, 
otherwise $\#A = \infty$. $A \subseteq B$ denotes ``$A$ is a subset of $B$'' (meaning that all elements of $A$ 
are also in $B$, e.g., $\mathbb{Q} \subseteq \mathbb{R}$).
...
Subsets of $ A \times B$ are called relations. We define a function $f \subseteq A
\times B$ from a set $A$ to a set $B$, as the special type of relation in which
for each element $a \in A$ there is a \textit{unique} element $b \in B$ such
that $(a, b) \in f$. In other words, for every element in $A$ we assign a
unique element in $B$. We usually write $f : A \to B$ to denote such a relation
and write $f(a) = b$ instead of writing down $(a,b) \in f$.

We say the function $f : A \to B$ is \textbf{injective} if condition $a_1 = a_2$
implies $f(a_1) = f(a_2)$. Function $f$ is said to be \textbf{surjective} if for
each $b \in B$ there exists $a \in A$ such that $f(a) = b$. A function that is
both injective and surjective is called a \textbf{bijection}.

% todo: add injective, surjective and bijective illustrations

\subsection{Logic}

Symbol $\forall$ means ``for all...''. For instance, $(\forall a \in A \subseteq \mathbb{R}): \{a < 1\}$ is 
...
Symbol $\exists$ means ``there exists such...''. Let us consider the following example: 
...
Symbol $\exists !$ means ``there exists a unique...''. For example, $(\exists! x \in \mathbb{R}_{>0}): \{x^2 = 4\}$ 
...
A \textbf{sequence} is a mapping from natural numbers $\mathbb{N}$ to some
specific set $X$. To denote the sequence, we write $\{x_n\}_{n \in \mathbb{N}}
\subseteq X$. If the sequence iterates through some specific set $\mathcal{I}
\subseteq \mathbb{N}$, we write $\{x_n\}_{n \in \mathcal{I}}$. For example,
$\{x_n\}_{n \in \{1,2,5\}} = (x_1, x_2, x_5)$. We write vectors in bold font,
e.g., $\mathbf{x} = (x_1, x_2, x_3) \in \mathbb{F}^3$, while matrices are
denoted by capital letters in regular font. Here is an example matrix: $A =
\begin{bmatrix} 1 & 2 \\ 3 & 4 \end{bmatrix} \in \mathbb{F}^{2 \times 2}$. For
additional linear algebra notation, refer to \Cref{section:linear-algebra},
where we introduce basics of linear algebra.
...
For example, instead of writing $\sum_{k=0}^{n-1}k$ we sometimes write $\sum_{k
\in [n]}k = \frac{n(n-1)}{2}$. Or, instead of specifying $n$ conditions
...
$\Pr[E] = \frac{1}{6}$. To denote the random \textit{uniform} sampling from the finite set $X$,
...
For instance, $\underset{x \leftarrowS [6]}{\Pr}[x \equiv 0 \; (\text{mod}\,2)]=\frac{1}{2}$.
