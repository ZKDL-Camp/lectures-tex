...
\mathbb{Z}_9$ we would like to know what it means to add them. Naturally, one
...
the following proposition to show the properties of modular arithmetic.
...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        behaves like ``zero'' in the addition operation (or like ``one'' when multiplying).
        \item $\textit{Additive inverse: } a + (-a) \equiv 0 \pmod{n}$. For any
        element $a$ we always have an element $-a$ which ``zeroes out'' an element $a$.
        \item $\textit{Addition commutativity: } a + b \equiv b + a \pmod{n}$. The order of elements in
...
\begin{remark}
    ...
    drastic exception: the existence of the inverse element. 
    For instance, this does not work in $\mathbb{Z}_4$ where $2$ has no multiplicative inverse.
\end{remark}

\vspace{-1.3mm}

Using algebraic terms, the previous proposition can be formulated concisely: \emph{$(\mathbb{Z}_n,+)$ is an abelian group}.
We will define this term shortly.

To summarize, in (abstract) algebra we study certain sets equipped with operations.
When we have a single properly defined operation, the set with this operation is called a \textit{group}.
When we have two operations (think of addition and multiplication), the set with these two operations is called a \textit{ring}.
Finally, if multiplication behaves nicely, we obtain a \textit{field}.

It then makes sense to study these sets with operations abstractly.
After all, we should not care what type of set we have underneath, be it a set of elliptic curve points or simply $\mathbb{Z}_n$.\footnote{Moreover, for cyclic elliptic curves these two groups are equivalent, thanks to \Cref{th:cyclic-group-isomorphism}.}
All theorems and facts we will develop will apply to both, which is surely convenient.

% \subsection*{What modern algebra studies?}

% \begin{remark}
%     Notes below are fairly complex, so one might skip them if this does not help.
% \end{remark}

% Interestingly, the modern algebra takes a step further.
% Instead of studying objects/sets, algebraists commonly study \textit{functions} (or in more fancy categorical terms, \textit{morphisms}) between algebraic structures.
% This, in turn, provide a much richer structure to study.

% As a concrete example, consider the set of points lying on the curve given by equation $f(x,y)=0$, \textit{i.e.}:
% \begin{equation*}
%     X(\mathbb{F}) = \{(x,y) \in \mathbb{F}^2: f(x,y) = 0\} \; \textcolor{gray}{\textit{// for some field $\mathbb{F}$ and $f \in \mathbb{F}[x,y]$.}}
% \end{equation*}

% Studying $X$ on its own is not very illuminating. 
% For instance, depending on the field $\mathbb{F}$, the set might be empty! (think of $\mathbb{F}=\mathbb{R}$ and $f(x,y)=x^2+y^2+1$). 
% Turns out that one might get much insight by considering an associated \textit{ring} instead, called a \textit{coordinate ring}:
% \begin{equation*}
%     \mathcal{O}_X := \mathbb{F}[x,y]/(f) \; \textcolor{gray}{\textit{// where $A/B$ denotes the quotient ring}}.
% \end{equation*}

% While $X(\mathbb{R})$ does not make sense for $f(x,y)=x^2+y^2+1$, it makes total sense to study $\mathbb{R}[x,y]/(x^2+y^2+1)$ instead.
% However, as mentioned early, algebraist take a step further. Instead of studying this ring, one studies ``nice functions'' over $X$:
% \begin{equation*}
%     \mathcal{O}_X(U) := \{f: U \to \overline{\mathbb{F}}, \; f \; \text{is a ``nice'' (regular) function on $U$}\}.
% \end{equation*}

% Here, $U \subseteq X$ is some ``nice'' set. 
% The association $U \mapsto \mathcal{O}_X(U)$ is called\footnote{Formally it is just a presheaf, yet it can be proven to be an actual sheaf.} a \textit{sheaf} and it is a main object in \textit{algebraic geometry}.

% For all previous discussion to make sense, one needs heavy preliminaries in category theory and commutative algebra.
% Fortunately, in this section we only deal with sets and operations over them (such as $X$ without considering $\mathcal{O}_X$ or $\mathcal{O}_X(U)$).
...
If you see this definition for the first time, you might be confused. 
That said, the best way to grasp this concept is to consider a couple of examples.

\begin{example}
    A group of integers with the regular addition $(\mathbb{Z},+)$ (also called the \textit{additive} group of integers) is a group. Indeed, an identity element is $e_{\mathbb{Z}}=0$, associativity obviously holds, and the inverse for each element $a \in \mathbb{Z}$ is given by $(\ominus a) := -a \in \mathbb{Z}$. 
\end{example}

\begin{remark}
    We use the term \textbf{additive group} when we mean that the binary operation is addition $+$, while \textbf{multiplicative group} means that we are multiplying two numbers via $\times$\footnote{In this section, $\cdot$ and $\times$ denote the same operation of multiplication.}.
\end{remark}
...
\begin{example}
    The set of $2 \times 2$ matrices with real entries and determinant equal to $1$ (denoted by $\text{SL}_2(\mathbb{R})$) is a group with respect to matrix multiplication\footnote{If you do not know how to multiply matrices, do not worry. We will cover specifics further in \Cref{section:linear-algebra}.}. However, this group is not abelian! Take
    \begin{xequation*}
        A = \begin{bmatrix}
            1 & 1 \\
            0 & 1
    ...
    \begin{xequation*}
        ...
        \end{bmatrix},
    \end{xequation*}
    so clearly $AB \neq BA$ -- the elements of $\text{SL}_2(\mathbb{R})$ do not commute.
\end{example}
...
\begin{definition}
    The \textbf{order} of a finite group $\mathbb{G}$ is the number of elements in the group. We denote the order of a group by $|\mathbb{G}|$ (or less frequently, by $\#\mathbb{G}$).
\end{definition}
\begin{example}
...
\begin{proof}
\textit{Statement (i).} Suppose we have two identity elements $e_1, e_2 \in
...
immediately that $e_1=e_2$.

\textit{Statement (ii).} Take some element $g \in \mathbb{G}$ and suppose $a, b \in \mathbb{G}$ are both inverses of $g$. 
...
$\mathbb{H} \subseteq \mathbb{G}$. Of course, since all elements in $\mathbb{H}$ are still 
...
\begin{definition}
    A subset $\mathbb{H} \subseteq \mathbb{G}$ is called a \textbf{subgroup} of
...
simple yet powerful structure. In a cyclic group,
...
\begin{example}
    Let $\mathbb{Z}_{12}$ be the \emph{additive} group of integers modulo $12$. 
    Consider the element $2 \in \mathbb{Z}_{12}$. Then, the group generated by $2$ is
    \begin{xequation}
...
inverse for each element (since for each $g^n \in \langle g \rangle$ the inverse is given 
by $g^{-n} \in \langle g \rangle$).
...
\begin{definition}
    ...
    we denote it by $\text{ord}(g)$. If such integer does not exist, we write
    $\text{ord}(g)=\infty$.
\end{definition}
...
\textbf{Reasoning.} Suppose the generator of the group is $g$. Then, for any $a,b \in \mathbb{G}$ we have $ab = g^m g^n = g^{m+n} = g^{n+m} = g^n g^m = ba$.
...
\begin{lemma}\label{lemma:cyclic-group-order}
    ...
    $g^{m} = e$ holds if and only if $n \mid m$.
\end{lemma}
...
Lagrange's Theorem is one of the most important
results in finite group theory, considering all the consequences. Central to
understanding Lagrange's Theorem is the notion of a \textit{coset}. While 
...
\begin{example}
    ...
    \begin{center} % todo: fix top margin
        ...
        \item $2 + \mathbb{H} = 6 + \mathbb{H} = \{2, 6\}$
...
Finally, we are ready for Lagrange's Theorem. This is a fundamental result in group
 theory, crucial for understanding the structure of finite groups.

\begin{theorem}[Lagrange]
    If $\mathbb{H}$ is a subgroup of any finite group $\mathbb{G}$, 
...
\begin{corollary}
    ...
    then $\#\mathbb{H} \mid \#\mathbb{G}$.
\end{corollary}

\begin{proof}
By Lagrange's theorem, the order of the group $\mathbb{G}$ 
...
it follows directly that $\left| \mathbb{H} \right|$ divides $\left| \mathbb{G} \right|$.
\end{proof}

\begin{corollary}
    Let $\mathbb{G}$ be a finite group and $g \in \mathbb{G}$, 
    then $\text{ord}(g) \mid \#\mathbb{G}$. In other words, the order of $g$ must 
    divide the number of elements in $\mathbb{G}$. 
\end{corollary}
...
serves as a generalization of Euler's Theorem for an arbitrary finite group.
...
\begin{definition}
    A function $\varphi: \mathbb{G} \to \mathbb{H}$ is called a \textbf{homomorphism} if it is a function that preserves the group operation, that is, 
    \begin{xequation}
...
\begin{example}
    Consider the group of integers $(\mathbb{Z},+)$ and the group of integers modulo $12$ $(\mathbb{Z}_{12},+)$. The function $\pi: \mathbb{Z} \to \mathbb{Z}_{12}$ defined as $\pi(x) = x \;\text{mod}\; 12$ is a homomorphism. Indeed:
    \begin{xequation*}
        \pi(a+b) = (a+b) \;\text{mod}\; 12 = (a \;\text{mod}\; 12) + (b \;\text{mod}\; 12) = \pi(a) + \pi(b).
    \end{xequation*}

    However, $\pi$ is not an isomorphism, since it is not bijective. For example, $\pi(0) = \pi(12) = 0$.
    Such map is commonly called a \textit{canonical projection}.
\end{example}
...
\begin{theorem}\label{th:cyclic-group-isomorphism}
    Let $\mathbb{G} = \langle g \rangle$ be a finite cyclic group of order $n$. Then, $\mathbb{G} \cong \mathbb{Z}_n$.
\end{theorem}

\textbf{Proof idea.} Define a function $\varphi: \mathbb{Z}_n \to \mathbb{G}$ as $m \mapsto g^m$. One can prove that this is an isomorphism.

Here, it is quite evident that isomorphism tells us that the groups have the same structure. 
In fact, $\cong$ is an equivalence relation on the class of groups
(see the exercise below), which is precisely why it is standard
practice to call two isomorphic groups \textit{equivalent}.

\textbf{Exercise (*).} Prove that $\cong$ is an equivalence relation.

Using the notion of isomorphism, we can restate Chinese Remainder Theorem (\Cref{th:chinese_remainder_theorem}) in the algebraic form.
\begin{theorem}[Restated \Cref{th:chinese_remainder_theorem}]
    Let $N=n_1\dots n_k$ and $n_1,\dots,n_k$ be pairwise coprime. Then, the map
    $[x]_N \mapsto ([x]_{n_1}, \dots, [x]_{n_k})$ defines the isomorphism\footnote{Here, this is a \textit{ring} isomorphism, meaning, that it preserves both addition and multiplication: \textit{i.e.,} $\varphi(x+y)=\varphi(x)+\varphi(y)$, $\varphi(xy)=\varphi(x)\varphi(y)$, and $\varphi(1)=1$.}
    \begin{equation*}
...
\begin{definition}
    An \textbf{endomorphism} of a group $\mathbb{G}$ is a homomorphism $\qquad \varphi: \mathbb{G} \to \mathbb{G}$
    from $\mathbb{G}$ to itself.
\end{definition}
...
\begin{example}
    Given a group $\mathbb{G}$ and a fixed element $a \in \mathbb{G}$, the map $\varphi: x \mapsto axa^{-1}$ is an automorphism.
\end{example}

The last two definitions are especially frequently used in Elliptic Curves theory.

\subsection{Fields}

Although typically one introduces rings before fields, we believe that for the 
...
Finite fields are the core object in cryptography. Instead of real numbers or pure integers, we will almost always 
use finite fields.
...
\begin{remark}
    ...
    assumed that we need full arithmetic (including division) for the protocol.
\end{remark}
...
For that reason, we can code a group in Rust. We will also write a simple test to check whether the group is 
...
\textbf{Checking group validity.} Now observe the following: we get closure for free, since the compiler will check 
whether the return type of the operation is the same as the type of the group. However, there is no guarantee that 
associativity holds, and our identity element is valid at all. For that reason, we need to somehow additionally check 
the validity of implementation.

We propose to do the following: we will randomly sample three elements from the group 
$a,b,c \leftarrowS \mathbb{G}$ and check our three properties:
\begin{enumerate}
    ...
    G: Group + Samplable,
{
    for _ in 0..TESTS_NUMBER {
        ...
        assert!(associativity_holds, "Associativity does not hold");
        ...
        assert!(identity_holds, "Identity element does not hold");
        ...
        assert!(inverse_holds, "Inverse element does not hold");
    }
}
...
Finally, we define the tests and run them:
\begin{minted}{rust}
#[cfg(test)]
mod tests {
    use super::*;
...
composition of two rotations is not commutative, so we expect the abelian test to fail. However, the group is still 
...
\begin{xexercise}
    {Exercise 6.}
    {Which of the following is a \textbf{valid} automorphism $f: X \to X$?}
    {2}
    {
...
\begin{xexercise}
    {Exercise 7*.}
    {
        Denote by $\text{GL}_2(\mathbb{R})$ a set of $2\times 2$ invertible matrices with real entries. Define 
        two functions $\varphi: \text{GL}_2(\mathbb{R}) \to \mathbb{R}$:
        ...
        \begin{enumerate}
            \item $\varphi_1$ is a group homomorphism between multiplicative groups $(\text{GL}_2(\mathbb{R}), \times)$ and $(\mathbb{R}, \times)$.
            \item $\varphi_2$ is a group homomorphism between additive groups $(\text{GL}_2(\mathbb{R}), +)$ and $(\mathbb{R}, +)$.
        \end{enumerate}
...
\textbf{Exercise 9*.} Prove that any group of order $9$ is abelian\footnote{This result might be generalized for groups of order $p^2$ for $p$ being prime.}.
