...
Previously, our discussion revolved around the finite field $\mathbb{F}_p$ for a prime $p$. However, many protocols
need more than just a prime field. For example, elliptic curve pairings and certain STARK constructions require
extending $\mathbb{F}_p$ to, in a sense, the analogue of complex numbers. 
...
the set of pairs $(x,y) \in \mathbb{R}^2$ where addition is naturally defined as $(x_1,y_1)+(x_2,y_2)=(x_1+x_2,y_1+y_2)$, and the multiplication is:
\begin{xequation*}
...
Formally, the definition of the field extension is straightforward:
\begin{definition}
    Let $\mathbb{F}$ be a field and $\mathbb{K}$ be another field. We say that $\mathbb{K}$ is an \textbf{extension} of $\mathbb{F}$ if $\mathbb{F} \subset \mathbb{K}$ and $1_{\mathbb{F}} \in \mathbb{K}$ and we denote it as $\mathbb{K}/\mathbb{F}$.
\end{definition}
...
\begin{example}
    Consider $\mathbb{Q}(\sqrt{2}, i) = \{a+bi: a,b \in \mathbb{Q}(\sqrt{2})\}$ where $i^2=-1$. This is a field extension of $\mathbb{Q}(\sqrt{2})$ and, consequently, of $\mathbb{Q}$. An element is represented as:
    \begin{xequation*}
...
Now, we present a more general way to construct field extensions. Notice that when constructing $\mathbb{C}$, we used the magical element $i$ that satisfies $i^2=-1$. But here is another way to think of it.

Consider the set of polynomials $\mathbb{R}[T]$, then we pick $p(T):=T^2+1 \in \mathbb{R}[T]$ and ask you to find roots of $p(T)$. Of course, you would claim that this equation has no solutions over $\mathbb{R}$ and that is totally true. 
This is why mathematicians introduced a new element $i$ that we formally called the root of $T^2+1$. Note however, that $i$ is not a number in the traditional sense, but rather a fictional symbol that we artificially introduced to satisfy the equation.

Now, could we have picked another polynomial, say, $q(T) = T^2+4$? Sure! As long as its roots cannot be found in $\mathbb{R}$, we are good to go.

\begin{example}
    Suppose $\beta$ is the root of $q(T):=T^2+4$. Then we could have defined complex numbers as a set of $x+y\beta$ for $x,y \in \mathbb{R}$. In this case, multiplication, for example, would be defined a bit differently than in the case of $\mathbb{C}$:
    \begin{xequation}
        (x_1+y_1\beta) \cdot (x_2+y_2\beta) = (x_1x_2-4y_1y_2) + (x_1y_2+x_2y_1)\beta.
    \end{xequation}
\end{example}

We shifted to the polynomial consideration for a reason: now, instead of considering the complex number $\mathbb{C}$ as ``some'' tuple of real numbers $(c_0,c_1)$, now let us view it as a polynomial $c_0+c_1T$ modulo polynomial $T^2+1$. 

\begin{example}
    Indeed, take, for example, $p_1(T) := 1+2T$ and $p_2(T) := 2+3T$. Addition is performed as we are used to:
    \begin{xequation*}
        p_1 + p_2 = (1+2T) + (2+3T) = 3+5T,
    \end{xequation*}

    but multiplication is a bit different:
    \begin{xequation*}
        p_1p_2 = (1+2T) \cdot (2+3T) = 2 + 3T + 4T + 6T^2 = 6T^2 +7T + 2.
    \end{xequation*}

    Well, and what next? Recall that we are doing arithmetic modulo $T^2+1$ and for that reason, we divide the polynomial by $T^2+1$:
    \begin{align*}
        &6T^2+7T+2 = 6(T^2+1) + 7T - 4 \\
        &\implies (6T^2+7T+2) \,\text{mod}\, (T^2+1) = 7T-4,
    \end{align*}

    meaning that $p_1p_2 = 7T-4$. Now let us come back to our regular complex number representation and multiply $(1+2i)(2+3i)$. We get $2+3i+4i+6i^2=-4+7i$. 
    That is exactly the same result if we change $T$ to $i$ above! In fact, what we have observed is the fact that our polynomial quotient ring $\mathbb{R}[T]/(T^2+1)$ is \emph{isomorphic} to $\mathbb{C}$.
\end{example}

So, let us generalize this observation to any field $\mathbb{F}$ and any irreducible polynomial $\mu \in \mathbb{F}[T]$.

\begin{theorem}\label{th:quotient-ring}
    Let $\mathbb{F}$ be a field and $\mu(T)$ --- irreducible polynomial over $\mathbb{F}$ (sometimes called a \textbf{reduction polynomial}). 
    Consider a set of polynomials over $\mathbb{F}[T]$ modulo $\mu$, formally denoted as $\mathbb{F}[T]/(\mu)$. Then, $\mathbb{F}[T]/(\mu)$ is a field.
\end{theorem}

\begin{example}
    As we considered above, let $\mathbb{F}=\mathbb{R}$, $\mu(T)=T^2+1$, then $\mathbb{R}[T]/(T^2+1)$ (a set of polynomials modulo $T^2+1$) is a field.
\end{example}

\begin{example}
    Suppose $\mathbb{F}=\mathbb{Q}$ and $\mu(T) := T^2-2$. Then, $\mathbb{Q}[T]/(T^2-2)$ is a field isomorphic to $\mathbb{Q}(\sqrt{2})$, considered above.
\end{example}
...
\begin{remark}
    ...
    \begin{equation*}
        \begin{aligned}
            \phi(a+b) = \phi(a) \oplus \phi(b), \quad \phi(a\times b) = \phi(a) \otimes \phi(b), \quad \phi(1_{\mathbb{F}}) = 1_{\mathbb{K}}.
        \end{aligned}
    \end{equation*}
\end{remark}

\Cref{th:quotient-ring} corresponds to viewing complex numbers as a polynomial quotient ring $\mathbb{R}[T]/(T^2+1)$. But, we can give a theorem (aka definition) for our classical representation via magical root $i$ of $T^2+1$.

\begin{theorem}
    Let $\mathbb{F}$ be a field and $\mu \in \mathbb{F}[T]$ be an irreducible polynomial of degree $n$ and let $\mathbb{K} := \mathbb{F}[T]/(\mu)$. Let $\theta \in \mathbb{K}$ be the root of $\mu$ over $\mathbb{K}$. Then,
    \begin{equation*}
...
\begin{definition}
    Suppose $p$ is prime and $m \geq 2$. 
    Let $\mu \in \mathbb{F}_p[T]$ be an irreducible polynomial of degree $m$. 
    Then, elements of $\mathbb{F}_{p^m}$ are polynomials in $\mathbb{F}_p[T]^{\leq m}$. In other words,
    \begin{xequation*}
        \mathbb{F}_{p^m} = \{c_0+c_1T+\cdots+c_{m-1}T^{m-1}: c_0,\dots,c_{m-1} \in \mathbb{F}_p\},
    \end{xequation*}
    where all operations are performed modulo $\mu$.
\end{definition}
...
\begin{example}
    ...
    One might choose the following reduction polynomial: $\mu(T)=T^4+T+1$ (of degree $4$). Then, operations are performed in the following manner:
    \begin{itemize}
        \item Addition: $(T^3+T^2+1)+(T^2+T+1) = T^3+T$.
        \item Subtraction: $(T^3+T^2+1)-(T^2+T+1) = T^3+T$.
        \item Multiplication: $(T^3+T^2+1)\cdot(T^2+T+1)=T^2+1$ since:
        \begin{xequation*}
            (T^3+T^2+1)\cdot(T^2+T+1) = T^5+T+1 \; \text{mod} \; (T^4+T+1) = T^2+1
        \end{xequation*}
        \item Inversion: $(T^3+T^2+1)^{-1}=T^2$ since $(T^3+T^2+1)\cdot T^2 \; \text{mod} \; (T^4+T+1) = 1$.
    \end{itemize}
\end{example}

Now, in the subsequent sections, we would need to extend $\mathbb{F}_p$ at least to $\mathbb{F}_{p^2}$. A convenient choice, similarly to the complex numbers, is to take $\mu(T)=T^2+1$. However, in contrast to $\mathbb{R}$, equation $T^2=-1 \pmod{p}$ might have solutions over certain prime numbers $p$. Thus, we consider proposition below.

\begin{proposition}
    Let $p$ be an odd prime. Then $T^2+1$ is irreducible in $\mathbb{F}_p[T]$ if and only if $p \equiv 3 \pmod{4}$.
\end{proposition}
...
The non-zero elements of $\mathbb{F}_p$, denoted as $\mathbb{F}_p^{\times}$, form a multiplicative cyclic group\footnote{It is easy to see why it is a group, yet the proof that it is cyclic is more involved.}. In other words, there exist elements $g \in \mathbb{F}_p^{\times}$, called \textit{generators}, such that
\begin{equation*}
...
\begin{xequation*}
    F(T) = \prod_{i=1}^n (T-f_i)+1 = (T-f_1)(T-f_2)\cdots(T-f_n)+1.
\end{xequation*}

Clearly, $F$ is a non-constant polynomial and has no roots in $\mathbb{F}$, since for any $f \in \mathbb{F}$, one has $F(f)=1$. $\blacksquare$
...
\begin{exbox}{Exercises 4-9. Tower of Extensions}
