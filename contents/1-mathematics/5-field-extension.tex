...
Formally, definition of the field extension is straightforward:
\begin{definition}
...
\begin{theorem}
    Let $\mathbb{F}$ be a field and $\mu \in \mathbb{F}[x]$ is an irreducible polynomial of degree $n$ and let $\mathbb{K} := \mathbb{F}[x]/(\mu)$. Let $\theta \in \mathbb{K}$ be the root of $\mu$ over $\mathbb{K}$. Then,
    \begin{xequation}
