...
will need to prove. That being said, let us define what polynomial is.
...
\begin{example}
    The degree of the polynomial $p(x) = 1 + 2x + 3x^2$ is $2$. Consequently, in particular, $p \in
    \mathbb{F}_5[x]^{\leq 2}$.
\end{example}
...
\begin{example}
    For example, $x_0=1$ is a root of $p(x) = x^2+2$ over $\mathbb{F}_3$. On the other hand, $p(x) = (x-1)(x-2)$ ($q(x) = x - 2$ according to Theorem~\ref{th:root-div}).
\end{example}
...
\begin{example}
    ...
    The notation is similar to one used in integer division.
\end{example}
...
\begin{lemma}
    ...
    where $\delta_{ij}$ is the so-called \textit{Kronecker delta}. Moreover, $\{\ell_i\}_{i=1}^n$ form a \textit{basis} of $\mathbb{F}[x]^{\leq n}$ (see \Cref{section:linear-algebra} for the definition). Put simply, for any polynomial $p \in \mathbb{F}[x]^{\leq n}$ there exist unique coefficients $\alpha_0,\dots,\alpha_n \in \mathbb{F}$ such that
    \begin{xequation}
