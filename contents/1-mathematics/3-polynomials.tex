...
\begin{definition}
    A \textbf{polynomial} $p(x)$ is a function of the form
    \begin{xequation*}
...
\begin{definition}
    The \textbf{degree} of a polynomial $p(x) = c_0+c_1x+c_2x^2+\dots$ is the largest $k \in \mathbb{Z}_{\geq 0}$ such that $c_k \neq 0$. We denote the degree of a polynomial as $\deg p$. We also denote by $\mathbb{F}[x]^{\leq m} := \{p \in \mathbb{F}[x]: \deg p \leq m\}$.
\end{definition}

\begin{example}
    The degree of the polynomial $p(x) = 1 + 2x + 3x^2$ is $2$. Consequently, $p \in
    \mathbb{F}_5[x]^{\leq 2}$.
\end{example}
...
\begin{definition}
    Let $p(x) \in \mathbb{F}[x]$ be a polynomial of degree $\deg p \geq 1$. A field element $x_0 \in \mathbb{F}$ is called a \textbf{root} of $p(x)$ if $p(x_0) = 0$.
\end{definition}
...
\begin{theorem}\label{th:root-div}
    Let $p \in \mathbb{F}[x], \deg p \geq 1$. Then, $x_0 \in \mathbb{F}$ is a root of $p(x)$ if and only if there exists a polynomial $q(x)$ (with $\deg q = n-1$) such that
    \begin{xequation}
        p(x) = (x-x_0)q(x)
    \end{xequation}
\end{theorem}
...
\begin{example}
    For example, $x_0=1$ is a root of $p(x) = x^2+2$ over $\mathbb{F}_3$. Indeed, $p(x) = (x-1)(x-2)$ ($q(x) = x - 2$ according to Theorem~\ref{th:root-div}).
\end{example}
...
\begin{definition}
    A polynomial $f(x) \in \mathbb{F}[x]$ is said to be \textbf{irreducible} in $\mathbb{F}$ if there are no polynomials $g,h \in \mathbb{F}[x]$ both of positive degree such that $f = gh$.
\end{definition}

\begin{example}
    A polynomial $f(x) = x^2+16$ is irreducible in $\mathbb{R}$. On the other hand, $f(x) = x^2-2$ is not irreducible since $f(x) = (x-\sqrt{2})(x+\sqrt{2})$. 
\end{example}
...
Suppose that $p(x) = ax + b$ -- just a simple linear function over $\mathbb{R}$. Suppose that $p(x)$ intercepts $(0,0)$ and $(1,2)$. Then, you can easily say that $p(x) = 2x$. 
...
\begin{lemma}
    ...
    where $\delta_{ij}$ is the so-called \textit{Kronecker delta}. Moreover, $\{\ell_i\}_{i=0}^n$ form a \textit{basis} of $\mathbb{F}[x]^{\leq n}$ (see \Cref{section:linear-algebra} for the definition). Put simply, for any polynomial $p \in \mathbb{F}[x]^{\leq n}$ there exist unique coefficients $\alpha_0,\dots,\alpha_n \in \mathbb{F}$ such that
    \begin{xequation}
...
\begin{lemma}[Single Variable Schwartz-Zippel Lemma]\label{lemma:one-sz} Let
    $\mathbb{F}$ be a finite field and let $f \in \mathbb{F}[x]$ be a non-zero
    polynomial. Then,
    \begin{xequation}
        \underset{x \leftarrowS \mathbb{F}}{\text{Pr}}
        [f(x) = 0] \leq \frac{\deg f}{\#\mathbb{F}}.
    \end{xequation}
\end{lemma}
...
\begin{lemma}[Multivariable Schwartz-Zippel] \label{lemma:sz} Let $\mathbb{F}$
    be a finite field and $\mathbb{S} \subseteq \mathbb{F}^n$ be a finite
    $n$-dimensional subspace. Let $f: \mathbb{S} \to \mathbb{F}$ be a polynomial
    in $n$ variables. Then,
    
    \vspace{-1.1mm}
    
    \begin{xequation}
        \underset{\mathbf{x} \leftarrowS \mathbb{S}}{\text{Pr}}
        [f(\mathbf{x}) = 0] \leq \frac{\deg f}{\#\mathbb{S}}.
    \end{xequation}
\end{lemma}
...
\begin{itemize}
    ...
    \leftarrowS \mathbb{F}_q$ and define polynomial $\omega(x) := \alpha +
    \sum_{j=1}^{t-1}k_jx^j \in \mathbb{F}_q[x]^{<t}$. Then, compute
...
First, we will set up our finite field $\mathbb{F}_q$ and polynomial ring $\mathbb{F}_q[x]$.
...
\begin{xexercise}
    {Exercise 4*.}
    {
        ...
        \end{xequation}
        For which values of $n$ does polynomial $Q(x)$ divide $P_n(x)$
    }
    {1}
    {
        \item For all even $n$.
...
\begin{xexercise}
    ...
    \leftarrowS \mathbb{S}$ where $\mathbb{S} \subseteq \mathbb{F}_p^n$ is a
