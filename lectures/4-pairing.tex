...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item Bob verifies whether $P+Q = [2]G$.
    \end{enumerate}
    
    It is easy to verify the correctness of the scheme: suppose Alice is honest and she sends the correct values of $\alpha,\beta$, satisfying $\alpha+\beta=2$. Then, $P+Q = [\alpha]G + [\beta]G = [\alpha+\beta]G = [2]G$. Moreover, Bob cannot learn $\alpha,\beta$ since the computational discrete logarithm problem is hard.        
\end{example}
...
\begin{example}
    ...
    \begin{enumerate}
        \item Alice computes $P \gets [\alpha]G_1, Q \gets [\beta]G_2$ --- points on the curve.
