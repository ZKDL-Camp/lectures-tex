...
\begin{definition}
    Interaction between $\mathcal{P}$ and $\mathcal{V}$ consists of verifier's queries $(q_1,q_2,\dots,q_{\ell})$, prover's responses $(m_1,m_2,\dots,m_{\ell})$, and $\mathcal{V}$'s random coins $(r_1,r_2,\dots,r_{\ell})$. The view of the verifier $\mathcal{V}$, denoted as $\mathsf{view}_{\mathcal{V}}(\mathcal{P},\mathcal{V})[x]$, is a random variable $(q_1,m_1,r_1,q_2,m_2,r_2,\dots,q_{\ell},m_{\ell},r_{\ell})$ that is determined by the random coins of $\mathcal{V}$ and the messages of $\mathcal{P}$ after the interaction with the statement $x$. See Figure below.
\end{definition}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-1.5cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=2mm, fill=white]{Send $m_1$};

        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-2.5cm]verifier.south) coordinate (l2)--(l2-|prover) node[midway, above=2mm, fill=white]{Toss coin $r_1$ and send query $q_1$};

        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-3.5cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=2mm, fill=white]{Send $m_2$};

        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-4.5cm]verifier.south) coordinate (l2)--(l2-|prover) node[midway, above=2mm, fill=white]{Toss coin $r_2$ and send query $q_2$};
        
        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-6.5cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=2mm, fill=white]{Send $m_{\ell}$};

        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-7.5cm]verifier.south) coordinate (l2)--(l2-|prover) node[midway, above=2mm, fill=white]{Toss coin $r_{\ell}$ and send query $q_{\ell}$};
