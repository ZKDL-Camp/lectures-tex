...
Essentially, message recovery attacks are types of attacks that, from given cipher text, recover the message with significantly better probability then random guessing, which is obvious $1/|\mathcal{M}|$.
...
One of the reasons for doing this is to thoroughly demonstrate the concept of a security reduction, which is the primary method used to assess the security of systems.

Let us play the following attack game message recovery between adversary $\mathcal{A}$ and challenger $\mathcal{B}$.
For a given cipher $\mathcal{E} = (E, D)$, defined over $\mathcal{K}, \mathcal{M}, \mathcal{C}$, and for given adversary $\mathcal{A}$, the attack game proceeds as follows: 
...
\begin{equation*}
    \text{MRadv}[\mathcal{A}, \mathcal{E}] := \Bigg|\text{Pr}[W] - \frac{1}{|\mathcal{M}|}\Bigg|.
\end{equation*}
...
$\blacktriangleright$ 
We will demonstrate how to construct an efficient adversary $\mathcal{B}$, whose advantage
in the semantic security game is linked to the message recovery advantage of $\mathcal{A}$.
...
\begin{enumerate}
    \item Adversary $\mathcal{B}$ generates $m_0, m_1 \in \mathcal{M}$ and sends to SS challenger.
    \item SS challenger returns ciphertext $c$ which goes to adversary $\mathcal{A}$.
    \item $\mathcal{A}$ returns $\hat{m}$. If $\hat{m} = m_0$, then $b = 0$, otherwise $b = 1$.
    \item Sends $b$ to SS challenger. 
\end{enumerate}

Intuitively, this implies that $\text{SSadv}[\mathcal{B}, \mathcal{E}]$ is exactly equal to $\text{MRadv}[\mathcal{A}, \mathcal{E}]$. $\blacktriangleleft$

\begin{remark} 
    % todo ref
    More formal proof you can find in A Graduate Course in Applied Cryptography, Dan Boneh and Victor Shoup, 2023.
\end{remark}
...
% Most cryptosystems are based on computational hard problems such as integer factorization, discrete logarithm and other.
