...
For example, $gcd(26, 8) = gcd(18, 8) = gcd(10, 8) = gcd(2, 8)$ can be optimized to $gcd(26, 8) = gcd(26 mod 8, 8) => gcd(2, 8)$
...
\begin{lemma}
Let $\F_p$ be a field. Let $f(x_1, x_2, ..., x_n)$ be a polynomial of total degree $d$. Suppose that $f$ is not the zero polynomial. Let $S$ be
a finite subset of $\F_p$. Let $r_1, r_2, ... r_n$ be chosen at random uniformly and independently from $S$. Then the probability that 
...
Schwartz-Zippel says that the probability that probability that $f(r) = 0$ is $\le \frac{2}{3}$.
\end{example}

Given two polynomials $P, Q$ with  degree $d$ in a field $\F_p$, for $r \xleftarrow{R} \mathbb{F}_3$: $\Pr[P(r) == Q(r)] \le \frac{d}{p}$.
For large fields, where  $\frac{d}{p}$ is negligble, this property allows to succintly check equality of polinimials.
