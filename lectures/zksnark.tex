...
format of it - \textbf{Rank-1 Constrain System}, where the simpliest term is \textbf{constraint}. 
...
\begin{definition}
    ...
    \begin{itemize}
        \item Symmetry: $\forall u, v \in \mathbb{V}: \< u, v \> = \overline{\< v, u \>}$
        \item Linearity in the first argument: $\forall u, v, w \in \mathbb{V}, \forall a \in 
        \mathbb{F}: \< au + v, w \> = a \< u, w \> + \< v, w \>$
        \item Positivity: $\forall u \in \mathbb{V}: \< u, u \> \geq 0$ and 
...
\begin{example}
    ...
    \[u = (1, 2, 3), v = (2, 4, 3)\], then:
    $\<u, v\> = \sum_{i=1}^{3}u_iv_i = 2*1 + 2*4 + 3*3 = 2 + 8 + 9 = 19$
\end{example}
...
\begin{example}
    ...
    \begin{align*}
        ...
        x_2 \times x_3 &= mult \tag{2} \\
        x_1 \times mult &= selectMult \tag{3} \\
        (1 - x_1) \times (x_2 + x_3) &= r - selectMult \tag{4}
    \end{align*}

    For every constraint we need the coefficients vectors $a_i$, $b_i$, $c_i$, but all of them have
    the same witness vector $w$.

    The interesting thing is where to take a constant from. In our case, it's just 1 in the (4) 
    constraint. The solution is straightforward: by placing 1 in the witness vector, we can then 
    obtain any desired value by multiplying it by an appropriate coefficient which is $-1$ in our
    case.

    The witness vector:
    \[ w = (1, r, x_1, x_2, x_3, mult, selectMult) \]
    ...
    Now, let's use specific values to compute an example:
    \[ x_1 = 1, \quad x_2 = 3, \quad x_3 = 4 \]

    We get:
    \[ \text{mult} = x_2 \times x_3 = 3 \times 4 = 12 \]
    \[ \text{selectMult} = x_1 \times \text{mult} = 1 \times 12 = 12 \]
    \[ r = x_1 \times (x_2 \times x_3) + (1 - x_1) \times (x_2 + x_3) \]
    \[ r = 1 \times (3 \times 4) + (1 - 1) \times (3 + 4) = 12 \]

    Verifying the constraints:
    \begin{enumerate}
        \item \( x_1 \times x_1 = x_1 \quad (1 \times 1 = 1) \)
        \item \( x_2 \times x_3 = \text{mult} \quad (3 \times 4 = 12) \)
        \item \( x_1 \times \text{mult} = \text{selectMult} \quad (1 \times 12 = 12) \)
        \item \( (1 - x_1) \times (x_2 + x_3) = r - \text{selectMult} \quad (0 \times 7 = 12 - 12) \)
    \end{enumerate}

    The witness vector with these values:
    \[ w = (1, 12, 1, 3, 4, 12, 12) \]
\end{example}
...
\begin{lemma}
    Suppose there is a constraint $\langle a, w \rangle \times \langle b, w \rangle = \langle c, w \rangle$ with coefficient vectors
    $a$, $b$, $c$ and witness vector $w$. Then it can be expressed in the form:
    \[ w^T A w + c^T w = 0 \]
    Where $A$ is the outer product of vectors $a$, $b$ (denoted as $a \otimes b$), consequently 
    a \textbf{rank-1} matrix.
\end{lemma}

\textbf{Lemma proof.} Suppose there is a constraint $\langle a, w \rangle \times \langle b, w \rangle = \langle c, w \rangle$. Where
vectors $a$, $b$, $c$, and $w$ have length $n$. Let's expand the inner products: 
