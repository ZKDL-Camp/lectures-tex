...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
      ...
      \draw [dashed,line width=0.3mm] ([yshift=-0.5cm]prover.south) -- ([yshift=-10cm]prover.south);
      \draw [dashed,line width=0.3mm] ([yshift=-0.5cm]verifier.south) -- ([yshift=-10cm]verifier.south);

      \node[align=center,fill=white!5,thick,below=0.5cm of prover](prover-compute){
      \noindent\rule{3.5cm}{0.8pt}\\
      Compute: \( q(X) \leftarrow \frac{f(X)}{Z_{\Omega}(X)} \) \\ \( q \in \mathbb{F}_p^{(\leq d)}[X] \) \\
      \noindent\rule{3.5cm}{0.8pt}};

      \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-3cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=0mm]{Send \(com(q)\)};

      \node[align=center,fill=white!5,thick,below=3.5cm of verifier](verifier-random){
      \noindent\rule{3.5cm}{0.8pt}\\
      Generate random \(r \xleftarrow{R}\mathbb{F}_p^{(\leq d)} \) \\
      \noindent\rule{3.5cm}{0.8pt}};

      \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-5.5cm]verifier.south) coordinate (l2)--(l2-|prover) node[midway, above=0mm]{Query \(q\) and \(f\) at $r$};

      \node[align=center,fill=white!5,thick,below=6.0cm of prover](prover-query){
      \noindent\rule{3.5cm}{0.8pt}\\
      Compute: $q(r), f(r)$ \\
      \noindent\rule{3.5cm}{0.8pt}};

      \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-8cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=0mm]{Send \(q(r), f(r)\)};

      \node[align=center,fill=white!5,thick,below=8.5cm of verifier](verifier-accept){
      \noindent\rule{3.5cm}{0.8pt}\\
      Accept if $f(r) = q(r) \cdot Z_{\Omega}(r)$ \\
      \noindent\rule{3.5cm}{0.8pt}};
  \end{tikzpicture}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt, align=center] (prover) {\includegraphics[width=1.25cm]{lectures/images/common/prover.png}\\Prover $\mathcal{P}(f)$}; 
        \node[inner sep=0pt, align=center, right=5cm of prover] (verifier) {\includegraphics[width=1.25cm]{lectures/images/common/verifier.png}\\Verifier $\mathcal{V}(f)$}; 

        \draw [dashed,line width=0.3mm] ([yshift=-0.5cm]prover.south) -- ([yshift=-14cm]prover.south);
        \draw [dashed,line width=0.3mm] ([yshift=-0.5cm]verifier.south) -- ([yshift=-14cm]verifier.south);

        \node[align=center, fill=white!5, thick, below=0.5cm of prover] (prover-compute) {
            \noindent\rule{5cm}{0.8pt}\\
            Compute: $t(X) \in \mathbb{F}_p^{(\leq k)}$ \\ 
            $t_1(X) = t(\omega \cdot X) - t(X)f(\omega \cdot X)$ \\
            $q(X) = t_1(X)/(X^k - 1) \in \mathbb{F}_p^{(\leq d)}$ \\
            \noindent\rule{5cm}{0.8pt}
        };

        \draw[-{Stealth[length=3mm]}, line width=0.4mm] 
            ([yshift=-3.5cm]prover.south) coordinate (l2) --
            (l2-|verifier) node[midway, above=0mm]{Send \(com(t)\), \(com(q)\)};

        \node[align=center, fill=white!5, thick, below=4cm of verifier] (verifier-random) {
            \noindent\rule{5cm}{0.8pt}\\
            Generate random $r \xleftarrow{R}\mathbb{F}_p$ \\
            \noindent\rule{5cm}{0.8pt}
        };

        \draw[-{Stealth[length=3mm]}, line width=0.4mm] 
            ([yshift=-6cm]verifier.south) coordinate (l2) --
            (l2-|prover) node[midway, above=0mm]{Query $t(X)$ at $\omega^{k-1}, r, \omega r$};

        \node[align=center, fill=white!5, thick, below=6.5cm of prover] (prover-query-t) {
            \noindent\rule{5cm}{0.8pt}\\
            Compute: $t(\omega^{k-1}), t(r), t(\omega r)$ \\
            \noindent\rule{5cm}{0.8pt}
        };

        \draw[-{Stealth[length=3mm]}, line width=0.4mm] 
            ([yshift=-8.5cm]prover.south) coordinate (l3) --
            (l3-|verifier) node[midway, above=0mm]{Send $t(\omega^{k-1}), t(r), t(\omega r)$};

        \draw[-{Stealth[length=3mm]}, line width=0.4mm] 
            ([yshift=-9.5cm]verifier.south) coordinate (l4) --
            (l4-|prover) node[midway, above=0mm]{Query $q(X)$ at $r$, and $f(X)$ at $\omega r$};

        \node[align=center, fill=white!5, thick, below=10cm of prover] (prover-query-qf) {
            \noindent\rule{5cm}{0.8pt}\\
            Compute: $q(r), f(\omega r)$ \\
            \noindent\rule{5cm}{0.8pt}
        };

        \draw[-{Stealth[length=3mm]}, line width=0.4mm] 
            ([yshift=-12cm]prover.south) coordinate (l5) --
            (l5-|verifier) node[midway, above=0mm]{Send $q(r), f(\omega r)$};

        \node[align=center, fill=white!5, thick, below=12.5cm of verifier] (verifier-accept) {
            \noindent\rule{5cm}{0.8pt}\\
            Accept if $t(\omega^{k-1}) = 1$ and \\ 
            $t(\omega r) - t(r)f(\omega r) = q(r)(r^k - 1)$ \\
            \noindent\rule{5cm}{0.8pt}
        };
    \end{tikzpicture}
    \caption{Protocol between prover $\mathcal{P}$ and verifier $\mathcal{V}$ for Product Check.}
    \label{fig:polyn}
\end{figure}
