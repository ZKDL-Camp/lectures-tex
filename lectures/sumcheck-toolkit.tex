...
$\mathbf{read}$ set. For each write operation a corresponding tuple $\mathsf{(addr, val, counter + 1)}$ is added, with
counter incremented by one, to the $\mathbf{write}$ set.
...
\noindent We track a single \emph{row‑memory}. The algorithm performs three reads and two writes.
\begin{center}
    \centering
...
\begin{align*}
    ...
    = \{(1,6,1)\} &\cup \{(0,2,0), (1,5,0), (2,7,0), (3,9,0)\}
\end{align*}
and \emph{rejects}.\\
