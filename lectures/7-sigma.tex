...
\begin{remark}
    The name ``Sigma'' protocol comes from the fact that the ``shape'' of the message flow vaguely resembles the Greek letter $\Sigma$.
\end{remark}
...
\begin{equation*}
    \mathcal{R} = \left\{ (\beta, (u,v,w)) \in \mathbb{Z}_q \times \mathbb{G}^3: v = g^{\beta} \wedge w = u^{\beta} \right\}
\end{equation*}
...
\begin{equation*}
    \mathcal{R} = \left\{ (h,(t,\psi)) \in \mathbb{H} \times (\mathbb{T} \times \mathcal{F}): \psi(h) = t \right\}
\end{equation*}
...
Now, suppose we have the Sigma interactive protocol $(\mathcal{P}_0,\mathcal{V}_0)$ for one relation $\mathcal{R}_0 \subset \mathcal{W}_0 \times \mathcal{X}_0$ and another Sigma interactive protocol $(\mathcal{P}_1,\mathcal{V}_1)$ for another relation $\mathcal{R}_1 \subset \mathcal{W}_1 \times \mathcal{X}_1$. Now, we want to combine these two protocols into a single one. Namely, we want our prover to be able to convince the verifier that:
\begin{enumerate}
