...
\textbf{Exercise 2.} Denote $X := \{(x,y) \in \mathbb{Q}^2: xy = 1\}$. Sasha claims the following:
\begin{enumerate}
...
\textbf{Exercise 3.} Does a tuple $(\mathbb{Z},\oplus)$ with operation defined as $a \oplus b = a + b - 1$ define a group?
\begin{enumerate}[a)]
...
\textbf{Exercise 4.} Consider the Cartesian plane $\mathbb{R}^2$, where two coordinates are real numbers. For two points $A,B$ define the operation
...
\textbf{Exercise 9.} Which of the following is a valid endomorphism $f: X \to X$? 
\begin{enumerate}[a)]
    \item $X = [0,1]$, $f: x \mapsto x^2$.
    \item $X = [0,1]$, $f: x \mapsto x + 1$.
    \item $X = \mathbb{R}_{>0}$, $f = (x-1)^3$.
    \item $X = \mathbb{Q}$, $f: x \mapsto \sqrt{x}$.
\end{enumerate}
