...
\begin{document}
    ...
    \begin{frame}{Classical Proofs}
        \begin{columns}
            ...
            \begin{column}{0.6\textwidth}
                \begin{itemize}
                    \only<1-5>{\item First proofs you have probably encountered were \textbf{geometry proofs}.}
                    \only<2-5>{\item You were given \textbf{axioms} and you can prove certain \textbf{statements} $x$ using them.}
                    \only<3-5>{\item The proof $\pi$ is a sequence of logical steps that lead from axioms to the statement. Essentially, you have a witness $w$ that proves the statement.}
                    \only<4-5>{\item Your teacher is the \textbf{verifier} $\mathcal{V}$ who checks your proof, while you are the \textbf{prover} $\mathcal{P}$.}
                    \only<5>{\item This is a \textbf{classical proof} and in a sense, it is a \textbf{non-interactive proof}.}
                \end{itemize}
            \end{column}
            % Column 2    
    ...
    \begin{frame}{Motivation}
        \begin{alertblock}{Note}
            ...
            \begin{itemize}
                \only<1-3>{\item Proofs must be verified by computers. Therefore, we need to develop \textbf{mathematic framework} to be able to program them.}
                \only<2-3>{\item This leads to the question: what is \textbf{statement}? What is \textbf{proof}? What is \textbf{witness}? How to formally define them?}
                \only<3>{\item We need to formalize these concepts.}
            \end{itemize}
        \end{alertblock}
    ...
    \begin{frame}{The most basic setting}
        \begin{itemize}
            \only<1-4>{\item We have a \textbf{prover} $\mathcal{P}$ and a \textbf{verifier} $\mathcal{V}$.}
            \only<2-4>{\item Prover $\mathcal{P}$ wants to prove some statement $x$ to the verifier.}
            \only<3-4>{\item Prover $\mathcal{P}$ has a \textbf{witness} $w$ that contains all necessary information to prove the statement $x$. He sends $\pi$ as a proof.}
            \only<4>{\item Verifier $\mathcal{V}$ wants to be convinced that the statement $x$ is true.}
        \end{itemize}
    ...
    \begin{frame}{Language}
        \begin{definition}[Relation]
            Given two sets $\mathcal{X}$ and $\mathcal{Y}$, the \textbf{relation} is $\mathcal{R} \subseteq \mathcal{X} \times \mathcal{Y}$.\pause
            \begin{itemize}
        ...
        \pause\begin{definition}[Language of true statements]
    ...
    \begin{frame}{Language Example \#1: Semiprimes}
        \begin{example}[Product of Two Primes (Semiprimes)]
            \textbf{Claim:} number $n \in \mathbb{N}$ is the product of two prime numbers $w = (p,q) \in \mathbb{N} \times \mathbb{N}$. The \textbf{relation} is given by:\pause
            \begin{equation*}
                \mathcal{R} = \{ (n, p, q) \in \mathbb{N}^3: n = p \cdot q \; \text{where $p,q$ are primes} \}
            \end{equation*}
        
            \pause In this particular case, the \textbf{language of true statements} is defined as 
            \begin{equation*}
                \mathcal{L}_{\mathcal{R}} = \{n \in \mathbb{N}: \exists w=(p,q) \; \text{are primes such that}\; n = p\cdot q\}
            \end{equation*}
            
            \pause\begin{itemize}
                \item \textbf{Valid witness \#1:} $n=15 \in \mathcal{L}_{\mathcal{R}}$. Witness: $w=(3,5)$.\pause
                \item \textbf{Invalid witness:} $n=16 \not\in \mathcal{L}_{\mathcal{R}}$. There is no valid witness.\pause
                \item \textbf{Valid witness \#2:} $n=50252009 \in \mathcal{L}_{\mathcal{R}}$. Witness: $w=(5749,8741)$.\pause
            \end{itemize}
        \end{example}
    ...
    \begin{frame}{Solution!}
        \begin{columns}
            ...
            \begin{column}{0.4\textwidth}
                ...
                \begin{itemize}
                    \item \textbf{Interaction:} instead of \textbf{passively} receiving the proof, the verifier $\mathcal{V}$ can \textbf{interact} with the prover $\mathcal{P}$ by sending \textbf{challenges} and receiving \textbf{responses}.\pause
                    \item \textbf{Randomness:} $\mathcal{V}$ can send random coins (challenges) to the prover, which $\mathcal{P}$ can use to generate responses.\pause
                \end{itemize}
            \end{column}
            \begin{column}{0.6\textwidth}
                \begin{figure}
                    ...
                    \begin{tikzpicture}[scale=0.90, transform shape]
                        ...
                        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-1.5cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=0mm, fill=white]{Send $m_1$};\pause
                
                        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-2.25cm]verifier.south) coordinate (l2)--(l2-|prover) node[midway, above=1mm, fill=white]{Toss coin $r_1$, send query $q_1$};\pause
                
                        \draw[-{Stealth[length=3mm]},line width=0.4mm] ([yshift=-3.5cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=0mm, fill=white]{Send $m_2$};\pause
    ...
    \begin{frame}{Quadratic Residue Interactive Proof}
        \begin{block}{Problem Statement}
            \begin{itemize}
                ...
                \end{equation*}
                \item \textbf{Witness:} $w = \text{modular square root of $x$}$. \pause
            \end{itemize}
        \end{block}
    ...
    \begin{frame}{Quadratic Residue Interactive Proof: Analysis}
        \begin{block}{Interactive Protocol}
            \begin{enumerate}
                \item $\mathcal{P}$ samples $r \xleftarrow{R} \mathbb{Z}_N^{\times}$ and sends $a = r^2$ to $\mathcal{V}$.\pause
                \item $\mathcal{V}$ sends a random bit $b \in \{0,1\}$ to $\mathcal{P}$.\pause
                \item $\mathcal{P}$ sends $z = r \cdot w^b$ to $\mathcal{V}$.\pause
                \item $\mathcal{V}$ accepts if $z^2 = a \cdot x^b$, otherwise it rejects.\pause
                \item Repeat $\lambda \in \mathbb{N}$ times.\pause
            \end{enumerate}
        \end{block}

        \begin{lemma}
            The aforementioned protocol is \textbf{complete} and \textbf{sound}.\pause
        \end{lemma}

        \textbf{Completeness.} If $b=0$, then $z=r$ and thus $z^2=r^2=a$, check passes.

        \pause If $b=1$, then $z=rw$ and thus $z^2=r^2w^2=ax$, check passes.
    \end{frame}
    ...
    \begin{frame}{Quadratic Residue Interactive Proof: Analysis}
        ...
        \begin{theorem}
            For any prover $\mathcal{P}^*$ with $x \not\in \mathcal{L}_{\mathcal{R}}$, the probability of $\mathcal{V}$ accepting the proof is at most $1/2$.\pause 
        \end{theorem}
    ...
    \begin{frame}{Interactive Protocol Definition}
        \textcolor{gray}{$\langle \mathcal{P}, \mathcal{V} \rangle(x)$ reads as ``interaction between $\mathcal{P}$ and $\mathcal{V}$ on the statement $x$''.}\pause

        \begin{definition}
            A pair of algorithms $(\mathcal{P},\mathcal{V})$ is called an \textbf{interactive proof} for a language $\mathcal{L}_{\mathcal{R}}$ if $\mathcal{V}$ is a polynomial-time verifier and the following two properties hold:\pause
            \begin{itemize}
                \item \textbf{Completeness:} For any $x \in \mathcal{L}_{\mathcal{R}}$, $\text{Pr}[\langle \mathcal{P}, \mathcal{V} \rangle(x) = \mathsf{accept}]=1$.\pause
    ...
    \begin{frame}{Verifier's View}
        \begin{alertblock}{Question \#1}
            What has the verifier learned during the interaction?\pause
        \end{alertblock}

        \begin{itemize}
            \item First things first, he learned that the statement $x$ is true.\pause
            \item He also knows queries $(q_1,\dots,q_{\ell})$ and random coins $(r_1,\dots,r_{\ell})$ he tossed (since he is the one who has sent them).\pause
            \item Moreover, he knows the prover's messages $(m_1,m_2,\dots,m_{\ell})$.\pause
        \end{itemize}
        ...
        \pause\textcolor{purple}{\textbf{Fact:}} $\mathsf{view}_{\mathcal{V}}(\mathcal{P}, \mathcal{V})$ is a \textbf{random variable}.
    \end{frame}

    \begin{frame}{Verifier's View: Example}
        \begin{example}
            For QN test, set $N:=3 \times 2^{30} + 1$ (prime number), and $\mathcal{P}$ wants to convince that $1286091780 \in \mathcal{L}_R$. Conversation is the following:\pause
            \begin{enumerate}
                \item During the first round, $\mathcal{P}$ sends $672192003$ to $\mathcal{V}$.\pause
                \item $\mathcal{V}$ sends $b=0$ to $\mathcal{P}$.\pause
                \item $\mathcal{P}$ sends $2606437826$ to $\mathcal{V}$.\pause
                \item $\mathcal{V}$ verifies that indeed $2606437826^2 \equiv 672192003 \pmod{N}$.\pause
                \item During the second round, $\mathcal{P}$ sends $2619047580$ to $\mathcal{V}$.\pause
                \item $\mathcal{V}$ chooses $b=1$ and sends to $\mathcal{P}$.\pause
                \item $\mathcal{P}$ sends $1768388249$ to $\mathcal{V}$.\pause
                \item $\mathcal{V}$ verifies that $1768388249^2 \equiv 2619047580 \times 1286091780 \pmod{N}$.\pause
    ...
    \begin{frame}{Verifier's View: Example}
        \begin{example}
            ...
            \pause\begin{itemize}
                \item Essentially, this view is the same as you have witnessed.\pause
                \item You have not learned anything about $w$ that prover $\mathcal{P}$ knows.\pause
                \item The witness was $w = 3042517305$ and two randomnesses were $r_1 = 2606437826$ and $r_2 = 3023142760$.\pause
    ...
    \begin{frame}{Zero-Knowledge Formally: Simulation Paradigm}
        \begin{alertblock}{Question \#2}
            What does it mean that the protocol is zero-knowledge?\pause
        \end{alertblock}

        \begin{itemize}
            \item Protocol is zero-knowledge if, given the verifier's $\mathsf{view}_{\mathcal{V}}(\mathcal{P}, \mathcal{V})$, verifier cannot infer any information about the witness $w$.\pause
            \item What does it mean that verifier $\mathcal{V}$ learns nothing new? It means that this view could have been simulated by $\mathcal{V}$ \textit{without even running an interaction}.\pause
            \item Call the view after the real interaction as \textbf{real view}, while the view after the simulation as \textbf{simulated view}.\pause
        \end{itemize}
    ...
    \begin{frame}{Computational Indistinguishability}
        ...
        \pause\begin{definition}[Informal Computational Indistinguisability]
    ...
    \begin{frame}{Zero-Knowledge Formally (Kind of)}
        ...
        \pause\begin{definition}[Zero-Knowledge (ZK)]
    ...
    \begin{frame}{Proof of Knowledge: Why?}
        Now, the main issue with the above definition is that \textit{we have proven the statement correctness, but we have not proven that the prover \textbf{knows} the witness}. These are completely two distinct things!\pause
        ...
        \pause\begin{alertblock}{Question}
            What does it mean that $X \in \mathcal{L}_{\mathcal{R}}$?\pause
        \end{alertblock}
    ...
    \begin{frame}{Proof of Knowledge: Definition}
        \begin{enumerate}
            \item The knowledge of witness means that we can \textbf{extract} the witness while interacting with the prover.\pause
            \item Thus, there should be an algorithm called \textbf{extractor} $\mathcal{E}$ which can extract the witness $w$.\pause
            \item $\mathcal{E}$ is given more power than $\mathcal{V}$ (otherwise, if the protocol is zero-knowledge, we cannot extract $w$). $\mathcal{E}$ can \textbf{rewind} and \textbf{call} prover $\mathcal{P}$ multiple times.\pause
            \item Sometimes, this is referred to as ``extractor $\mathcal{E}$ uses $\mathcal{P}$ as an oracle''.\pause
        \end{enumerate}
    ...
    \begin{frame}{Random Oracle (RO)}
        ...
        \begin{definition}[Informal definition of RO]
            Suppose someone is inputting $x$ to the random oracle $\mathcal{O}_{\text{R}}: \mathcal{X} \to \mathcal{Y}$\footnote{Typically, RO works with a family of functions $f: \mathcal{X} \to \mathcal{Y}$, but we are not going too deep into the details.}. The oracle $\mathcal{O}_{\text{R}}$ does the following:\pause
            \begin{enumerate}
                \item If $x$ has been queried before, the oracle returns the same value as it returned before.\pause
                \item If $x$ has not been queried before, the oracle returns a randomly uniformly sampled value from the output space $\mathcal{Y}$.\pause
            \end{enumerate}
        \end{definition}
    ...
    \begin{frame}{Fiat-Shamir Transformation}
        ...
        One of such transformations is called \textbf{Fiat-Shamir heuristic}. Idea:\pause
        \begin{enumerate}
            \item If all what $\mathcal{V}$ does is sending uniformly random values, this is an overkill.\pause
            \item Instead of $\mathcal{V}$ sending random values, prover should be able to generate it himself, but he should not know the randomness in advance.\pause
    ...
    \begin{frame}{Fiat-Shamir Heuristic Illustration}
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[scale=0.70, transform shape]
                ...
                % Sending proof
                \pause
x                \draw[-{Stealth[length=3mm]},line width=0.4mm,color=green!50!black] ([yshift=-8.5cm]prover.south) coordinate (l2)--(l2-|verifier) node[midway, above=0mm, fill=white]{Send $\pi = (m_1,r_1,m_2,r_2,\dots,m_{\ell},r_{\ell})$};
            \end{tikzpicture}
