...
\begin{document}
    ...
    \pause\begin{example}
        Consider $f(x) = x^3+2$ and $g(x) = x+1$ over $\mathbb{R}$. Then, we can write $f(x) = (x^2-x+1)g(x) + 1$, so the remainder of the division is $r \equiv 1$. \pause Typically, we denote this as:
        \begin{equation*}
    ...
    \begin{frame}{Application: Shamir Secret Sharing}
      ...
      \begin{definition}
        \textbf{Secret Sharing} scheme is a pair of efficient algorithms $(\mathsf{Gen}, \mathsf{Comb})$ which work as follows:\pause
        \begin{itemize}
