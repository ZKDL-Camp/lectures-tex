...
\textbf{Exercise 1.} Alice and Bob were watching a horse race. 
...
\textbf{Exercise 2.} Bob made a setup (points $G$ and $U$) for a Pedersen commitment scheme and commited values $(m, r) = (3, 7)$ to Alice
by sending him $\mathcal{C} = [3]G + [7]U$. Alice did not verify the setup. Turns out that Bob knows that $U$ = $[6]G$. 
Bob is planning to send a different message from the one he originally committed to
to $m_2 = 15$. Which values $(m_2, r_2)$ should he send to Alice at the opening stage?
\vspace{-3mm}
\begin{multicols}{4}
    \begin{enumerate}[(a)]
        \item $(15, 5)$
...
\textbf{Exercise 3.} Given a polynomial $p(x) = x^3 - 10x^2 + 31x - 30$, Alice wants to prove that $p(2)=0$. 
...
what value of $q(x)$ has Alice calculated?
