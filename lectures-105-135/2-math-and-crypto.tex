...
\begin{definition}
    The encryption scheme $\mathcal{E}$ with a security paramter $\lambda \in \mathbb{N}$ is \textbf{semantically secure} if for any efficient adversary $\mathcal{A}$ the following value is negligible.
    \begin{equation*}
...
\begin{definition}[GCD and LCM for multiple arguments]
    ...
    \begin{align*}
        \gcd(a_1, \dots, a_n) &= \max\{ d \in \mathbb{N}: d \mid a_1, \dots, d \mid a_n \}. \\
        \lcm(a_1, \dots, a_n) &= \min\{ m \in \mathbb{N}: a_1 \mid m, \dots, a_n \mid m \}.
    \end{align*}
\end{definition}
...
\exerciseTitle{Exercise 1.}{Suppose that for the given cipher with a security parameter $\lambda$, the adversary $\mathcal{A}$ can deduce the least significant bit of the plaintext from the ciphertext. Recall that the advantage  of a bit-guessing game is defined as $\text{SS}\mathsf{Adv}[\mathcal{A}] = \left|\Pr[b=\hat{b}] - \frac{1}{2}\right|$, where $b$ is the randomly chosen bit of a challenger, while  $\hat{b}$ is the adversary's guess. What is the maximal advantage of $\mathcal{A}$ in this case?}
\exerciseTitle{Hint:}{The adversary can choose which messages to send to challenger to further distinguish the plaintexts.}
\vspace{0.5mm}
\exerciseMulticols{3} {
    \item $1$
    \item $\frac{1}{2}$
...
\exerciseTitle{Exercise 3.}{Suppose that $f$ and $g$ are negligible functions. Which of the following functions is not neccessarily negligible?}
\exerciseAnswerList {
    \item $f + g$
    \item $f \times g$
    \item $f - g$
...
\exerciseTitle{Exercise 4.}{Suppose that $f \in \mathbb{F}_p[x]$ is a $d$-degree polynomial with $d$ \textbf{distinct} roots in $\mathbb{F}_p$. What is the probability that, when evaluating $f$ at $n$ random points, the polynomial will be zero at all of them?}
\exerciseMulticols{2} {
    \item Exactly $(d/p)^n$.
...
\exerciseTitle{Question 5.}{Suppose that you received the encoded message $(3,5,6,9)$. Which number from the encoded message is corrupted?}
\exerciseMulticols{2} {
...
\exerciseTitle{Question 6.}{Consider the previous question. Suppose that the original message was $(a,b)$. Find the value of $a \times b$ (in $\mathbb{F}_{13}$).}
\exerciseMulticols{5} {
    \item $4$
    \item $6$
    \item $12$
