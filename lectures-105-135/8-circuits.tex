...
which are then used to create and verify proofs.

That being said, let us introduce the \textbf{preprocessing NARK}.
...
\begin{itemize}[left=0mm]
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            &\text{Pr}[\mathsf{Verify}(\mathsf{vp}, x, \mathsf{Prove}(\mathsf{pp},x,w)) = \mathsf{accept}] = 1            
        \end{aligned}
    \end{equation*}
...
\begin{definition}[Knowledge Soundness]
    $\Pi_{\text{preNARK}}$ is (adaptively) \textbf{knowledge sound} for a relation $\mathcal{R}$ if for every PPT adversary $\mathcal{A}=(\mathcal{A}_0,\mathcal{A}_1)$, split into two algorithms, such the following value greater then $\alpha$:
    \begin{equation*}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \hspace{0.04\textwidth} % Space between figures
    \begin{minipage}{0.46\textwidth}
        \centering
...
\begin{example}
    Consider the Hadamard product of matrices: 
    $A = \begin{bmatrix}
        1 & 1 & 2 \\
        3 & 0 & 3
...
\begin{example}
    ...
    \begin{equation*}
        \begin{aligned}
            &(a_1w_1 + a_2w_2 + a_3w_3)(b_1w_1 + b_2w_2 + b_3w_3) = \\
...
\begin{example}
    ...
    \begin{align*}
        \mathbf{a}_1 &= (0, 0, 1, 0, 0, 0, 0) & \quad \mathbf{b}_1 &= (0, 0, 1, 0, 0, 0, 0) \\
        \mathbf{a}_2 &= (0, 0, 0, 1, 0, 0, 0) & \quad \mathbf{b}_2 &= (0, 0, 0, 0, 1, 0, 0) \\
        \mathbf{a}_3 &= (0, 0, 1, 0, 0, 0, 0) & \quad \mathbf{b}_3 &= (0, 0, 0, 0, 0, 1, 0) \\
        \mathbf{a}_4 &= (1, 0, -1, 0, 0, 0, 0) & \quad \mathbf{b}_4 &= (0, 0, 0, 1, 1, 0, 0) 
    \end{align*}

    \begin{align*}
        \mathbf{c}_1 &= (0, 0, 1, 0, 0, 0, 0) \\
        \mathbf{c}_2 &= (0, 0, 0, 0, 0, 1, 0) \\
        \quad \mathbf{c}_3 &= (0, 0, 0, 0, 0, 0, 1) \\
        \quad \mathbf{c}_4 &= (0, 1, 0, 0, 0, 0, -1)
    \end{align*}
